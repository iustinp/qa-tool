...
function loadPears(runDir, file = 'source-clm.json') {
  ...
  for (const d of fs.readdirSync(runDir, { withFileTypes: true })) {
    ...
    try {
      ...
      if (Array.isArray(j.nodes) && j.nodes.length) out.push({ url: j.url || d.name, nodes: j.nodes, clickables: Array.isArray(j.clickables) ? j.clickables : [], dir: path.join(runDir, d.name), width: j.width, height: j.height, dpr: j.dpr || 1 });
