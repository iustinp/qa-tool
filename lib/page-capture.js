...
async function capturePage(url, options = {}) {
  const {
    ...
    profiles = ['desktop'],
    captureOptions = {},
    collectTextGeometry = false,
    collectCanonicalLayout = false,
    logCaptureStep,
  } = options;
  ...
  for (const profName of profiles) {
    ...
    const cap = await captureFullPageBuffer(url, {
      ...
      // Text-layout fingerprint for the deterministic layout audit (both sides).
      collectTextGeometry,
      // Canonical Layout Model (pear) for the canonical layout comparison.
    ...
    pageCapture.profiles[profile.name] = {
      ...
      textGeometry: meta.textGeometry || null,
      canonicalLayout: meta.canonicalLayout || null,
