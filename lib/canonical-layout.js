...
async function extractCanonicalLayout(page) {
  ...
  const json = await page.evaluate(async () => {
    ...
    const hiddenNodes = [];
    const hiddenItems = [];
    const offItems = [...scrollItems].filter(
      (it) => isHiddenByClipOrOffscreen(it) && !anyHiddenAncestor(it) && !isInClosedDetails(it)
    );
    if (offItems.length) {
      const savedStyle = [];
      ...
      offItems.forEach((it, idx) => {
        const ir = it.getBoundingClientRect();
        if (ir.width < 6 || ir.height < 6) return;
        const clickable = it.matches(CLICK_SEL) || !!it.querySelector(CLICK_SEL);
        hiddenItems.push({
          idx,
          x: Math.round(ir.left + window.scrollX),
          y: Math.round(ir.top + window.scrollY),
          w: Math.round(ir.width),
          h: Math.round(ir.height),
          clickable,
        });
        // Text runs within this off-screen item, at their natural positions —
        // same shape as nodes[] so the review renders them identically.
