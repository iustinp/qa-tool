  ...
  const json = await page.evaluate(async () => {
    ...
    const anyHiddenAncestor = (el) => {
      for (let p = el; p; p = p.parentElement) {
        // A recipe-ignored subtree is treated as not present: excluded from every
        // pear (text/media/bg-image) exactly like a hidden ancestor would be.
        if (p.nodeType === 1 && (!isVisible(p) || p.hasAttribute('data-ppd-ignore'))) return true;
      }
      return false;
    };
    ...
    // Inline-paragraph identity: the nearest block-level ancestor of a text node.
    // Runs inside one <p> (bold lead-in, links, plain continuation) share a `bid`,
    // so the compare step can treat a reflowing inline paragraph as ONE unit and not
    // score every run past a divergent wrap point as separate drift. Ids are per this
    // capture (source & target number independently — matched pairs group by source).
    const blockId = new Map();
    let nextBid = 0;
    const nearestBlock = (el) => {
      for (let cur = el; cur && cur !== document.body; cur = cur.parentElement) {
        const disp = window.getComputedStyle(cur).display;
        if (disp && !disp.startsWith('inline') && disp !== 'contents') return cur;
      }
    ...
    const bidOf = (el) => {
      ...
      if (id === undefined) { id = nextBid++; blockId.set(block, id); }
      return id;
    };
    ...
    for (let node = walker.nextNode(); node; node = walker.nextNode()) {
      ...
      // First rendered line's left. A run that starts mid-line (after a bold lead-in)
      // and WRAPS gets a union box (rect) whose left snaps to the paragraph margin;
      // the first client rect is where the text visually begins on line 1. We keep the
      // union box as the node geometry (matching/drift rely on it) but carry the line-1
      // inset so the pear can text-indent line 1 back to its true start instead of
      // painting it from the margin, on top of the lead-in.
      const rects = range.getClientRects();
      const firstLeft = rects.length ? rects[0].left : rect.left;
      const st = window.getComputedStyle(el);
      nodes.push({
        kind: 'text',
        ...
        indent: Math.max(0, Math.round(firstLeft - rect.left)),
        bid: bidOf(el),
    ...
    // --- Elements painted via background-image: decorative leaf art (icons) AND section /
    // hero backgrounds (containers). This used to be restricted to childless, textless leaves
    // "to avoid huge containers", which silently dropped every hero / section background image
    // (a container that holds a heading + button). We now capture containers too, guarded:
    //   - skip <html>/<body> (page-level background, not a block);
    //   - skip a real url() only (a linear-gradient overlay is not an image);
    //   - skip full-page-height wrappers (>=85% of the document — not a section);
    //   - dedupe nested/inherited duplicates of the SAME src (a section + its transparent inner
    //     div) down to the outermost (largest) node.
    const bgCands = [];
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (rect.height >= docH * 0.85) return;
      bgCands.push({
        src: m[1], position: st.position,
        x: Math.round(rect.left + window.scrollX), y: Math.round(rect.top + window.scrollY),
        w: Math.round(rect.width), h: Math.round(rect.height),
      });
    });
    bgCands.sort((a, b) => b.w * b.h - a.w * a.h); // largest first, so the outermost is kept
    const bgKept = [];
    for (const c of bgCands) {
      const dup = bgKept.some((k) => k.src === c.src &&
        c.x >= k.x - 2 && c.y >= k.y - 2 && c.x + c.w <= k.x + k.w + 2 && c.y + c.h <= k.y + k.h + 2);
      if (!dup) bgKept.push(c);
    }
    for (const c of bgKept) nodes.push({ kind: 'bg-image', src: c.src, x: c.x, y: c.y, w: c.w, h: c.h, position: c.position });
    ...
    return JSON.stringify({
      ...
      width: docW,
      height: docH,
      // Device-pixel-ratio in effect (profile deviceScaleFactor). The review divides
      // the screenshot's natural pixel dims by this to get its true document CSS size,
      // so the shot is displayed 1:1 with the pear coordinate space even when the
      // page's document width differs from `width` (e.g. horizontal overflow that
      // appears between extraction and the full-page screenshot). #grace-shot-shrink.
  ...
  for (const n of clm.nodes) {
    ...
    if (n.kind === 'text') {
      ...
      const style =
        ...
        // Line-1 inset for a run that starts mid-line and wraps (see extraction): indent
        // only the first line so it begins at its true x, wrapped lines fall to the margin.
        (n.indent ? `text-indent:${n.indent}px;` : '') +
