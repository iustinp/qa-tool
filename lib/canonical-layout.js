...
async function extractCanonicalLayout(page) {
  ...
  const json = await page.evaluate(async () => {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      // Find the inner TRACK by STRUCTURE, not scrollWidth (a 2-card track that exactly
      // fills the container has scrollWidth==clientWidth while the container overflows
      // only via its nav arrows — so scrollWidth misses it and grabs the whole row as
      // one item). Descend into a child that spans most of the container AND itself
      // holds >=2 item children (a Swiper .swiper-wrapper, a flex row); repeat. Else the
      // current sizable children are the items.
      ...
      let hasItem = false;
      for (const ch of bigKids(track)) { scrollItems.add(ch); hasItem = true; }
      if (hasItem) scrollContainers.add(el);
    }
    const clickables = [];
