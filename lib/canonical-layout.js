...
async function extractCanonicalLayout(page) {
  ...
  const json = await page.evaluate(async () => {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      if (!/(auto|scroll|hidden)/.test(cs.overflowX)) continue; // clips horizontally (a carousel does)
      if (el.clientWidth < 40) continue;
      // Gate to actual carousels: it must OVERFLOW, or carry a delegated click listener
      // (data-ppd-click, i.e. the site made it interactive — cards opening a modal).
      // Without this every overflow:hidden container with a similar row (grids, image
      // crops) matched and exploded the trigger count.
      if (el.scrollWidth <= el.clientWidth + 2 && !el.hasAttribute('data-ppd-click')) continue;
      // A carousel needn't OVERFLOW — a row of 3 cards that fit is still a carousel (a
      // tab-switch swaps in others) and its cards must each be a click target. So detect
      // by STRUCTURE, not scrollWidth (a track exactly filling the container reports
      // scrollWidth==clientWidth): descend to the inner TRACK (a child spanning most of
      // the container that holds >=2 item children — a Swiper .swiper-wrapper, a flex
      // row), then require its children to be a horizontal ROW of >=2 SIMILAR-sized items
      // (the carousel signature; guards against generic clipped / 2-column content).
      const bigKids = (e) => [...e.children].filter((ch) => { const r = ch.getBoundingClientRect(); return r.width >= 40 && r.height >= 40; });
      let track = el;
      for (let d = 0; d < 8; d += 1) {
        const inner = bigKids(track).find((ch) => ch.getBoundingClientRect().width >= track.clientWidth * 0.7 && bigKids(ch).length >= 2);
        if (!inner) break;
        track = inner;
      }
      const items = bigKids(track);
      if (items.length < 2) continue;
      const rects = items.map((i) => i.getBoundingClientRect());
      if (new Set(rects.map((r) => Math.round(r.left / 8))).size < 2) continue; // stacked, not a horizontal strip
      const ws = rects.map((r) => r.width);
      if (Math.max(...ws) > Math.min(...ws) * 1.8) continue; // dissimilar widths → not a card row
      for (const ch of items) scrollItems.add(ch);
    ...
    // Then other clickables: outermost-only, and SKIP a scroll container (or a
    // wrapper of scroll items) — its item cards above are the real click targets, so
    // we don't also emit the whole-row container box.
    document.querySelectorAll(CLICK_SEL).forEach((el) => {
      ...
      let wrapsItem = false;
      for (const it of scrollItems) { if (it !== el && el.contains(it)) { wrapsItem = true; break; } }
      if (wrapsItem) return;
      pushClk(el);
    });
