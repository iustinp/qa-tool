...
async function extractCanonicalLayout(page) {
  ...
  const json = await page.evaluate(async () => {
    ...
    for (const el of document.querySelectorAll('*')) {
      ...
      // to the container's width by rect, but its content width stays wide. If no
      // child overflows, the items are the direct children.
      let track = el;
      for (let d = 0; d < 8; d += 1) {
        let next = null;
      ...
      for (const ch of track.children) {
        ...
        if (r.width >= 40 && r.height >= 40) { scrollItems.add(ch); hasItem = true; }
      }
