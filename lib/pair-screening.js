...
function loadScreeningThresholds() {
  return {
    ...
    // Regional (banded) image diff — localizes *where* pages differ. Additive:
    // does not affect the pass/fail/needs_ai verdict (still global similarity).
    bandCount: envInt('PPD_SCREEN_BANDS', 8),
    bandChangedMaxSimilarity: envFloat('PPD_SCREEN_BAND_CHANGED_MAX', 0.9),
  };
}
...
async function screenPair(sourceMeta, targetMeta, sourceBuffer, targetBuffer, opts = {}) {
  ...
  const imageResult = await compareImages(sourceBuffer, targetBuffer, {
    ...
    bandCount: thresholds.bandCount,
    bandChangedMaxSimilarity: thresholds.bandChangedMaxSimilarity,
  });

  const scores = {
    imageSimilarity: imageResult.similarity,
    imageDiffRatio: imageResult.diffRatio,
    imageBandCount: imageResult.bandCount,
    imageChangedBandCount: imageResult.changedBandCount,
    imageBands: imageResult.bands,
