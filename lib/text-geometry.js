...
function toReadingOrder(items, lineTol) {
  ...
  // Reading-order left = the run's TRUE first-line start, not its box left. A run
  // that begins mid-line (after a bold lead-in or an inline link) and then WRAPS
  // has a union box whose left snaps to the paragraph margin; ordering by that box
  // left puts the wrapped continuation BEFORE the inline it actually follows,
  // reversing reading order and breaking alignment. `indent` (first client-rect
  // left − box left, captured at extraction) restores the visual start.
  const rl = (i) => i.x + (i.indent || 0);
  // Sort by y first so we can group into lines.
  withMid.sort((a, b) => a.y - b.y || rl(a) - rl(b));
  const lines = [];
...
/**
 * Coalesce inline-split text runs: adjacent items on the same line, of the same
 * font size, separated only by a small (sub-space) gap, are one phrase the
 * markup happened to split into multiple nodes ("Mayura Metal" + "Credit Card").
 * Merging them before matching keeps segmentation consistent across two
 * differently-authored pages. The gap threshold is deliberately tight (a
 * fraction of an em) so genuinely separate items — nav links, table cells,
 * anything with real spacing — are left alone.
 * @param {Array} ordered items already in reading order
 */
function coalesceInlineRuns(ordered, opts = {}) {
  const maxGapEm = opts.coalesceMaxGapEm ?? 0.6;
  const out = [];
  for (const it of ordered) {
    const prev = out[out.length - 1];
    if (prev) {
      const sharedH = Math.min(prev.h, it.h);
      const sameLine = Math.abs(prev.y + prev.h / 2 - (it.y + it.h / 2)) <= sharedH * 0.5;
      const sameSize = Math.abs((prev.fontSize || 0) - (it.fontSize || 0)) <= 1;
      const gap = it.x - (prev.x + prev.w);
      const em = it.fontSize || prev.fontSize || 16;
      if (sameLine && sameSize && gap >= -3 && gap <= maxGapEm * em) {
        const right = Math.max(prev.x + prev.w, it.x + it.w);
        const bottom = Math.max(prev.y + prev.h, it.y + it.h);
        prev.text = `${prev.text} ${it.text}`.replace(/\s+/g, ' ').trim();
        prev.norm = normalizeTextLine(prev.text);
        prev.w = right - prev.x;
 ...
 * @param {{ minChars?: number, lineTol?: number, coalesceInline?: boolean }} [opts]
 */
function buildFingerprint(rawItems, opts = {}) {
  ...
  let ordered = toReadingOrder(cleaned, opts.lineTol);
  if (opts.coalesceInline) ordered = coalesceInlineRuns(ordered, opts);
  return ordered.map((it, i) => {
    ...
    return {
      ...
      x: it.x,
      y: it.y,
      // Inline-paragraph id (nearest block ancestor) — groups runs so drift is scored
      // per paragraph, not per reflowed run. See canonical-layout.js.
      ...(it.bid != null ? { bid: it.bid } : {}),
      w: it.w,
      h: it.h,
