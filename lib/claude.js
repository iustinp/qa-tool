...
function imageBlock(base64Png, mediaType = 'image/png') {
  return {
    type: 'image',
...
async function visionJson(userParts, maxTokens = 4096) {
  const content = userParts.map((p) => {
    if (p.type === 'image_buffer') {
      return imageBlock(p.buffer.toString('base64'), p.mediaType || 'image/png');
    }
    if (p.type === 'text') return textBlock(p.text);
