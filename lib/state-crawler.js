  ...
  return page.evaluate(
    ({ SEL, ITEM, DBG }) => {
      ...
      const addCand = (el, srcTag) => {
        ...
        const isCrossPage = (an) => {
          const h = ((an && an.getAttribute('href')) || '').trim().toLowerCase();
          return !!h && !h.startsWith('#') && !h.startsWith('javascript:');
        };
        const a = el.closest('a[href]');
        if (a && isCrossPage(a)) {
          if (rec) { rec.skip = 'cross-page-anchor(' + (a.getAttribute('href') || '').trim().slice(0, 24) + ')'; dbg.push(rec); }
          return;
        }
        // Skip a LINK-ONLY container: ANY element (not just a "card" — a div, li,
        // span, slide, whatever) whose only actionable content is a cross-page link
        // that WRAPS an <a href> to another page, with no click handler of its own.
        // closest() above only catches the trigger being INSIDE an anchor; this is
        // the inverse — a container that CONTAINS the anchor — so it slips through
        // and gets clicked to a no-op that then picks up unrelated occlusion/drift
        // noise as a bogus "reveal". Skip ONLY when the container has NO in-page
        // behaviour of its own or inside it: it (or a descendant) must NOT be a
        // delegated-click target (data-ppd-click) or an explicit control (button/
        // summary/aria-expanded/aria-controls/role=button|tab|menuitem/onclick/#hash
        // anchor). A container that carries its own click handler is INDISTINGUISHABLE
        // from a modal-opener (a benefit card that opens a detail modal looks identical
        // to an "explore other" card that navigates — same self data-ppd-click + a
        // cross-page link), so we keep it rather than risk dropping real in-page
        // reveals (a false content-MISSING). Only the handler-less link wrapper is
        // unambiguous, and it's the actual no-op source.
        if ([...el.querySelectorAll('a[href]')].some(isCrossPage)) {
          const inPageSel =
            'button, summary, [aria-expanded], [aria-controls], [role=button], [role=tab], [role=menuitem], [onclick], a[href^="#"]';
          // check the element ITSELF (self attrs — matches()) AND its descendants
          let inPage = el.hasAttribute('data-ppd-click') || el.matches(inPageSel) || !!el.querySelector(inPageSel);
          if (!inPage) {
            for (const c of el.querySelectorAll('[data-ppd-click]')) {
              if (!isCrossPage(c.closest('a[href]'))) { inPage = true; break; }
            }
          }
          if (!inPage) {
            if (rec) { rec.skip = 'link-only'; dbg.push(rec); }
            return;
          }
        }
        if (!vis(el)) {
