  ...
  return page.evaluate(
    ({ SEL, ITEM, DBG }) => {
      ...
      document.querySelectorAll(SEL).forEach((el) => {
        ...
        if (!el.matches(ITEM) && el.parentElement && el.parentElement.closest(SEL)) {
          if (rec) {
            const anc = el.parentElement.closest(SEL);
            rec.skip = 'shadowed-by <' + (anc && anc.tagName) + '> .' + ((anc && anc.className) || '').toString().slice(0, 30) + (anc && anc.hasAttribute('data-ppd-click') ? ' [ppd-click]' : '');
            dbg.push(rec);
          }
          return;
        }
