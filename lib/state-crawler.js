...
async function crawlStates(page, opts = {}) {
  ...
  for (let i = 0; i < Math.min(count, maxTriggers); i++) {
    ...
    // the page changed — otherwise open panels/modals accumulate and a captured
    // state would show several unrelated things at once. Only in state-capture
    // mode (a reload per state-changing click); the text-only crawl accumulates.
    ...
    if (captureStates && clickNew.length) dirty = true; // state changed → reset before next
