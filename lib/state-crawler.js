...
async function crawlStates(page, opts = {}) {
    ...
    try {
      ...
      try {
        await page.waitForFunction(
          ...
          { timeout: opts.resetHeaderWaitMs ?? 4000 }
        );
      } catch {
      ...
      await page.waitForTimeout(capOpts.stabilizeSettleMs ?? 400);
      await installNavBlocker(page);
  ...
  let localIdx = 0;
  while (true) {
    const i = shared ? shared.nextIdx++ : localIdx++; // pull next trigger from the (shared) queue
    if (i >= limit) break;
...
function resolveMaxWorkers(opts) {
  ...
  // Default: one worker per CPU thread (each worker is a browser, so threads are
  // the natural bound), hard-capped at 10. crawlUrlParallel further clamps to the
  // trigger count. Override with opts.maxWorkers / PPD_CRAWL_WORKERS.
  return Math.max(1, Math.min(req || cpu, 10));
}
...
async function crawlUrlParallel(url, opts = {}) {
  ...
  const runWorker = (workerId) => crawlUrl(url, { ...opts, shared, workerId }).catch(() => ({}));
