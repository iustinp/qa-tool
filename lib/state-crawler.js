...
// Triggers we click — interactive, but not cross-page NOR in-page navigation.
// Anchor links (href="#..." / javascript:) are deliberately NOT here: an in-page
// anchor just scrolls (revealing nothing new — and the scroll makes fixed/sticky
// headers appear to move, which would fool the position-delta signal). A genuinely
// interactive anchor (with its own JS click handler) is still caught via
// [data-ppd-click], so we lose only pure scroll-to links.
const TRIGGER_SEL = [
  'button',
  'summary',
  '[role=button]',
  ...
  return page.evaluate(() => {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (a) {
        ...
        // Block the DEFAULT for cross-page links AND in-page (#) anchors — the
        // latter's only effect is a scroll-jump, which reveals nothing and would
        // shift fixed/sticky headers past the position-delta signal. Any JS click
        // handler on the element still runs; only the anchor's own scroll/nav is
        // suppressed. (javascript: hrefs are left to execute.)
        if (h && !h.toLowerCase().startsWith('javascript:')) e.preventDefault();
      }
    }, true);
