...
// Carousel internals (.swiper-slide cards, .swiper-button-next,
// .swiper-pagination-bullet) are intentionally NOT here: advancing to hidden
// slides chases content we can't fully enumerate and makes source/target
// inconsistent. A carousel is judged on its VISIBLE cards (already in the base
// pear); tagTriggers additionally skips anything inside a .swiper-slide.
const TRIGGER_SEL = [
  'button',
  'summary',
  '[role=button]',
...
// another trigger (e.g. a [role=tab] inside a container that carries a delegated
// click listener). The outermost-only rule is bypassed for these, otherwise the
// container would shadow every tab.
...
function tagTriggers(page, sel, itemSel) {
  return page.evaluate(
    ({ SEL, ITEM }) => {
      // Visible enough to click. We deliberately DON'T apply the CLM's full
      // clip/off-screen rule here — it over-excluded legitimate below-fold
      // triggers (FAQ tabs, footer accordions). The hidden-carousel case is
      // already handled by skipping .swiper-slide, and non-active tab panels are
      // display:none. We only add the closed-<details> rule (collapsed accordion
      // content is hidden except its summary).
      const vis = (el) => {
      ...
      document.querySelectorAll(SEL).forEach((el) => {
        // Skip navigating anchors — clicking them leaves the page (some navigate
        // via JS even past the nav-blocker), they're not in-place reveals.
        const a = el.closest('a[href]');
        if (a) {
          const h = (a.getAttribute('href') || '').trim().toLowerCase();
          if (h && !h.startsWith('#') && !h.startsWith('javascript:')) return;
        }
        // Carousel cards: skip the slide and anything inside it (however it
        // matched — cards often carry a data-ppd-click listener). We don't crawl
        // into carousels; their visible cards are already in the base pear.
        if (el.matches('.swiper-slide') || el.closest('.swiper-slide')) return;
...
async function crawlStates(page, opts = {}) {
  ...
  const seen = new Set(baseHarvest); // grows: a revealed line is attributed to its first trigger
  ...
  for (let i = 0; i < Math.min(count, maxTriggers); i++) {
    ...
    // Everything from record → click → capture → undo is guarded: a click can
    // navigate (some anchors/buttons route via JS past the nav-blocker) and
    // destroy the execution context, which would otherwise throw out of an
    // evaluate and crash the whole crawl. On any failure we recover to a clean
    // base and move on.
    try {
      ...
      // Keep the observer live through the settle so async (fetched) modal
      // content is recorded too, and thus reversible.
      ...
      // Likely a navigation that destroyed the context. If we left the page,
      // reload to base; otherwise best-effort undo. Either way, keep crawling.
      if (page.url() !== baseUrl) {
        navigations += 1;
        await reloadToBase();
      } else {
        try {
          await page.evaluate(() => window.__ppdMutUndo && window.__ppdMutUndo());
        } catch {
          /* ignore */
        }
      }
