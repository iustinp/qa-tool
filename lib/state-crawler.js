  ...
  return page.evaluate(
    ({ SEL, ITEM, DBG }) => {
      ...
      document.querySelectorAll(SEL).forEach((el) => {
        ...
        // NOTE (#62): the old "outermost-only" rule (skip an element nested inside
        // another trigger) was REMOVED — it shadowed genuine inner triggers (nav /
        // menu items, cards inside a delegated-listener container), so their content
        // was never explored. Nav is content present on EVERY page and must be
        // verified, so nested triggers are now tagged too. Duplicate / no-op clicks
        // are merged by state dedup; per-node budget + dedup bound the state count.
        // (The cross-page-anchor and visibility skips still apply.)
        if (!vis(el)) {
          if (rec) { rec.skip = 'not-visible'; dbg.push(rec); }
          return;
        }
        el.setAttribute('data-ppd-idx', String(i));
        if (rec) { rec.tagged = i; dbg.push(rec); }
        i += 1;
      });
