  ...
  const explore = async (curStable, curSig, depth, parentKeys = new Set()) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const resetScroll = async () => {
        await timed('reset', () =>
          page.evaluate(() => {
            // Restore any strip/carousel scroll changed by scrollIntoView-before-click,
            // so sibling triggers are measured in the pristine layout and the base view
            // isn't left scrolled. Done HERE (after reveal detection), not right after
            // the click: some tab widgets re-derive the active tab from scroll, so an
            // immediate restore would revert the selection before the reveal is seen.
            try { for (const [p, l, t] of window.__ppdScrollSave || []) { p.scrollLeft = l; p.scrollTop = t; } window.__ppdScrollSave = null; } catch { /* ignore */ }
      ...
      await timed('click', async () => {
        ...
        // Bring the trigger into view ONLY IF a coordinate click would MISS it — i.e.
        // it's not the topmost element at its own centre (clipped out of a scroll
        // strip, so the strip's clip/background is on top). Then scrollIntoView within
        // any scroll ancestor (`inline:'center'` scrolls the strip like a user;
        // `block:'nearest'` keeps the occlusion-centred vertical scroll) and remember
        // the ancestors' scroll to RESTORE later (else it corrupts sibling geometry +
        // the base view). CRUCIAL: skip this when the trigger IS already hittable —
        // scrollIntoView disturbs swiper/carousel widgets (snaps the active slide, so
        // sibling tabs reveal the same content and wrongly dedup). Then use a FRESH box.
        try {
          await handle.evaluate((el) => {
            window.__ppdScrollSave = null;
            const r = el.getBoundingClientRect();
            ...
            if (cx >= 0 && cy >= 0 && cx <= window.innerWidth && cy <= window.innerHeight) {
              ...
              // Hittable = the click at the centre lands ON the trigger or a DESCENDANT.
              // NOT top.contains(el): a clipped trigger's overflow CONTAINER is the
              // topmost painted thing at that point and DOM-contains the (clipped)
              // trigger — treating that as hittable would skip the scroll and miss.
              hittable = !!top && (top === el || el.contains(top));
            }
            if (hittable) return; // already clickable at its centre — don't disturb it
            const save = [];
            for (let p = el.parentElement; p; p = p.parentElement) {
              if (p.scrollWidth > p.clientWidth || p.scrollHeight > p.clientHeight) save.push([p, p.scrollLeft, p.scrollTop]);
            }
            window.__ppdScrollSave = save;
        ...
        // The strip/carousel scroll is left as-is here so the reveal is detected with
        // the clicked tab still selected (and the state screenshot shows it in view);
        // resetScroll() restores the ancestors' scroll afterwards, keeping later
        // triggers' geometry + the base view pristine.
      });
