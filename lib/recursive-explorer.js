  ...
  const explore = async (curStable, curSig, depth, parentKeys = new Set()) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      await timed('click', async () => {
        ...
        // GHOST a hidden trigger for the review: image the element WHEREVER it is —
        // including an off-screen carousel card — via Playwright's element screenshot
        // (it scrolls the element into view, unlike a plain viewport clip). Save+restore
        // all ancestor + window scroll around it so the reveal detection that follows
        // sees exactly the same state. Best-effort with a short timeout (a transform-only
        // swiper slide it can't reveal just yields no ghost → an empty box, still clickable).
        if (geom.hidden) {
          try {
            const gw = await page.evaluate(() => [window.scrollX || 0, window.scrollY || 0]);
            // Render the element for the shot even if it's a TRANSFORM-positioned swiper
            // slide that's off-screen (Playwright's scroll-into-view can't reveal those —
            // they move by transform, not scroll): temporarily neutralise overflow AND
            // transform on its ancestors (whole inline style saved) so it lays out at its
            // natural spot, let Playwright scroll to + image just it, then restore.
            await handle.evaluate((el) => {
              window.__ppdGhost = [];
              for (let p = el.parentElement; p; p = p.parentElement) {
            ...
            // Drop a blank capture (a slide even this couldn't render → near-uniform
            // pixels): leave a clickable box, not a misleading empty ghost.
            let blank = false;
