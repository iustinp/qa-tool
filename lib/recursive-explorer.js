      ...
      await timed('click', async () => {
        ...
        try {
          didUnclip = await handle.evaluate((el) => {
            ...
            if (hittable) return false; // already clickable — don't disturb it
            const save = [];
        ...
        // This trigger was CLIPPED (we had to un-clip to reach it) → it isn't visible
        // in the base backdrop. Flag it and grab a small screenshot of the element
        // (now un-clipped + in view, pre-click) so the review can ghost it in place.
        if (didUnclip && box && box.width >= 1 && box.height >= 1 && box.x >= 0 && box.y >= 0 && box.x + box.width <= vp.width && box.y + box.height <= vp.height) {
          geom.hidden = true;
          try {
