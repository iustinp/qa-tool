...
async function exploreStates(page, opts = {}) {
  ...
  const explore = async (curStable, curSig, depth) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      // carousel slide-swap (equal add+remove) now registers. Jaccard overlap is
      // kept for DEDUP (findVisited) below, where comparing sibling states of
      // similar size is exactly what it's good at.
      const minAdded = opts.minAdded ?? 2;
