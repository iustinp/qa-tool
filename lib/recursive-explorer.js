...
async function pixelReveal(page, beforeBuf, beforeRaw, volatile, opts = {}) {
  ...
  try {
    ...
    afterRaw = await pixelDiff.frame(afterBuf);
  } catch {
  ...
  for (const r of det.regions.slice(0, maxRegions)) {
    const clip = {
      ...
      width: Math.min(r.width, afterRaw.w - Math.max(0, r.left)),
      height: Math.min(r.height, afterRaw.h - Math.max(0, r.top)),
    };
...
async function exploreStates(page, opts = {}) {
  ...
  const explore = async (curStable, curPos, curSig, depth, parentKeys = new Set()) => {
    ...
    if (pixelDetect) {
      try {
        ...
        parentRaw = await pixelDiff.frame(parentBuf);
      } catch {
  ...
  if (pixelDetect) {
    try {
      const frames = [];
      for (let i = 0; i < 3; i++) {
        frames.push(await pixelDiff.frame(await page.screenshot({ fullPage: true, type: 'png' })));
