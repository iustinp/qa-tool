  ...
  const explore = async (curStable, curSig, depth, parentKeys = new Set()) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      await timed('click', async () => {
        ...
        // First bring the trigger into view HORIZONTALLY within any scroll ancestor
        // (a tab strip / carousel with overflow-x): a tab clipped out of its strip
        // still has a doc-space box, so the coordinate click would land on the strip's
        // clip/background = a false noop. `inline:'center'` scrolls the strip to the
        // tab like a user would; `block:'nearest'` leaves the vertical scroll (already
        // centred for the occlusion before/after) undisturbed. Remember the scroll
        // ancestors' positions so we can RESTORE them after the click — otherwise the
        // strip stays scrolled and corrupts later triggers' recorded geometry (and
        // leaves the base view's strip off-position). Then use a FRESH box.
        try {
          await handle.evaluate((el) => {
            const save = [];
