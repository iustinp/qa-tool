...
async function exploreStates(page, opts = {}) {
  ...
  const captureState = async (info, addedLines, sig, depth) => {
    let clm = null;
    let shot = null;
    if (captureStates) {
      try {
  ...
  const explore = async (curStable, curSig, depth) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      if (clickErr || page.url() !== baseUrl) {
        ...
        edges.push({ fromSig: curSig.hash, toSig: null, triggerLabel: info.label, kind: 'nav' });
        await reloadToBase();
      ...
      if (quickAdded === 0) {
        ...
        edges.push({ fromSig: curSig.hash, toSig: curSig.hash, triggerLabel: info.label, kind: 'noop' });
        continue;
      }
      ...
      if (!edge.changed) {
        // The added lines were volatile noise; not a real state change.
        await page.evaluate(() => window.__ppdMutUndo());
        edges.push({ fromSig: curSig.hash, toSig: curSig.hash, triggerLabel: info.label, kind: 'noop' });
        continue;
      }

      const known = findVisited(childStable, visited, matchOpts);
      if (known) {
        edges.push({ fromSig: curSig.hash, toSig: known.key.hash, triggerLabel: info.label, kind: 'known' });
        await page.evaluate(() => window.__ppdMutUndo());
        continue;
      }

      // A genuinely new state. Remember it, capture it, and recurse into it.
      const childSig = signatureKey(childStable);
      visited.push({ key: childSig, stable: childStable });
      await captureState(info, edge.added, childSig, depth + 1);
      edges.push({ fromSig: curSig.hash, toSig: childSig.hash, triggerLabel: info.label, kind: 'new' });
  ...
  // Seed: the base state itself.
  const { stable: baseStable } = await sampleStableText(page, sampleOpts);
  const baseSig = signatureKey(baseStable);
  visited.push({ key: baseSig, stable: baseStable });
