...
async function exploreStates(page, opts = {}) {
  ...
  const clog = (...a) => { if (CLICK_DEBUG) console.error('[clickdbg]', ...a); };
  ...
  const explore = async (curStable, curSig, depth, parentKeys = new Set()) => {
    ...
    if (CLICK_DEBUG) {
      try {
        ...
        clog(`d${depth} tagTriggers: ${tagged.length} tagged / ${dbg.length} candidates`);
        for (const d of dbg) {
          const how = d.tagged != null ? `TAG#${d.tagged}` : `skip: ${d.skip}`;
          clog(`  ${how}  <${d.tag}${d.ppd ? ' ppd' : ''}> "${d.label}" ${d.href ? 'href=' + d.href : ''} .${d.cls}`);
        }
      } catch (e) {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      if (clickErr || page.url() !== baseUrl) {
        ...
        clog(`  -> NAV/ERR (url now ${page.url() !== baseUrl ? page.url() : 'same'}, clickErr=${clickErr}) — reload to base`);
        pushEdge(null, 'nav');
        await reloadToBase();
      ...
      if (quickAdded === 0) {
        if (!(await tryPixelState())) {
          clog('  -> NOOP (quick-gate: no added text, pixel flat)');
          noopGate += 1;
      ...
      if (edge.added.length < minAdded && edge.removed.length < minAdded) {
        ...
        if (!(await tryPixelState())) {
          clog('  -> NOOP (stable-sample: added/removed below threshold, pixel flat)');
          noopSig += 1;
          pushEdge(curSig.hash, 'noop');
        }
        await page.evaluate(() => window.__ppdMutUndo());
