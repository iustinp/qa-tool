 ...
 */
async function pixelReveal(page, beforeBuf, beforeRaw, volatile, opts = {}) {
  ...
  let afterRaw;
  try {
    afterBuf = await page.screenshot({ fullPage: true, type: 'png' });
    afterRaw = await pixelDiff.frame(afterBuf, { scale });
  } catch {
    return { changed: false, revealed: [] };
  }
  const det = pixelDiff.detect(beforeRaw, afterRaw, volatile, { minBlocks: opts.pixelMinBlocks ?? 4 });
...
async function exploreStates(page, opts = {}) {
  ...
  // #62 Phase A — TALL VIEWPORT for the crawl. Viewport-at-clickable state
  // backdrops (below) shoot the VIEWPORT, not the full page; a taller viewport
  // captures more vertical content per shot so a revealed modal/panel is far less
  // likely to fall below the fold. WIDTH is preserved unchanged from the base
  // capture: width drives responsive breakpoints AND keeps crawl trigger geometry
  // aligned with the pre-crawl base pear (same layout). Only HEIGHT grows — this
  // changes coverage, not layout. The base pear+screenshot were already captured
  // (pre-onPageReady), so this resize never touches them.
  try {
    const vp = page.viewportSize();
    if (vp && vp.width) {
      const tallH = Math.max(Math.round(vp.width * (opts.viewportTallRatio ?? 1.35)), vp.height || 0);
      if (tallH > (vp.height || 0)) await page.setViewportSize({ width: vp.width, height: tallH });
    }
  } catch {
    /* viewport tweak is best-effort; fall back to the base viewport */
  }
  ...
  const explore = async (curStable, curPos, curSig, depth, parentKeys = new Set()) => {
    ...
    // This state's rendered frame — the "before" for the pixel second-opinion on
    // any text-flat click at this level (page is in this state now; siblings revert
    // back to it, so one frame serves them all).
    let parentBuf = null;
    let parentRaw = null;
    if (pixelDetect) {
      try {
        parentBuf = await page.screenshot({ fullPage: true, type: 'png' });
        parentRaw = await pixelDiff.frame(parentBuf, { scale: pixelScale });
      } catch {
        /* pixel path degrades to off */
      }
    }
    ...
    for (let i = 0; i < limit; i++) {
      ...
      try {
        ...
        for (const o of quick) {
          ...
          else if (!o.f) quickPos[k] = { x: o.x, y: o.y }; // skip fixed/sticky
        }
      ...
      const tryPixelState = async () => {
        if (!pixelDetect || !parentRaw) return false;
        const pr = await pixelReveal(page, parentBuf, parentRaw, volatile, opts);
      ...
      if (edge.added.length < minAdded && edge.removed.length < minAdded && edge.moved.length < minMoved) {
        // No stable text delta AND no genuine rearrangement — but the pixels might
        // still have changed (occlusion/visual-only), so ask the pixel second-opinion
  ...
  if (pixelDetect) {
    try {
      ...
      volatile = pixelDiff.volatileFromFrames(frames, {});
    } catch {
      volatile = null;
    }
  }
