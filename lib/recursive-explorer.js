  ...
  const explore = async (curStable, curSig, depth, parentKeys = new Set()) => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      await timed('click', async () => {
        ...
        let didUnclip = false;
        try {
          didUnclip = await handle.evaluate((el) => {
            window.__ppdUnclip = null;
            const r = el.getBoundingClientRect();
            if (r.width < 1 || r.height < 1) return false;
            const cx = r.left + r.width / 2;
            const cy = r.top + r.height / 2;
            // Only un-clip when the centre is IN the viewport but something else is on
            // top (true horizontal clip / occlusion). If the centre is OFF-viewport, the
            // element is merely mis-positioned (e.g. vertical scroll thrown off by a
            // scroll-animation transform) — NOT clipped; un-clipping wouldn't help and
            // would wrongly flag a visible tab as hidden. Leave those to the click below.
            if (cx < 0 || cy < 0 || cx > window.innerWidth || cy > window.innerHeight) return false;
            const top = document.elementFromPoint(cx, cy);
            // Hittable = the click lands ON the trigger or a DESCENDANT. NOT
            // top.contains(el): a clipped trigger's overflow CONTAINER is the topmost
            // painted thing there and DOM-contains the (clipped) trigger.
            if (top && (top === el || el.contains(top))) return false; // already clickable — don't disturb
            const save = [];
        ...
        // This trigger was CLIPPED (we had to un-clip to reach it) → it isn't in the
        // base backdrop. Flag it (box drawn in the review) and, while it's un-clipped
        // and in the viewport, grab a small screenshot for the review to ghost it in
        // place. No scrolling — un-clip already made it hittable at its true spot.
        if (didUnclip) {
          geom.hidden = true;
          if (box && box.width >= 1 && box.height >= 1 && box.x >= 0 && box.y >= 0 && box.x + box.width <= vp.width && box.y + box.height <= vp.height) {
            try {
              const gbuf = await page.screenshot({ type: 'png', clip: { x: box.x, y: box.y, width: box.width, height: box.height } });
              geom.ghost = `data:image/png;base64,${gbuf.toString('base64')}`;
