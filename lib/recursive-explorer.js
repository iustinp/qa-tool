  ...
  try {
    ...
    const json = await handle.evaluate((el) => {
      ...
      return JSON.stringify({
        ...
        clipped: el.hasAttribute('data-ppd-clip'),
      });
    });
...
/**
 * HOVER-REVEAL triggers (a mega-menu opened by CSS :hover, no click — common in
 * header/nav bars; the top-level item itself is often a bare <li> with no href/
 * ARIA/click-listener at all, so it never matches TRIGGER_SEL, and even where it
 * DOES match (an aria-expanded li), clicking it is the wrong model: a real click
 * would navigate on a genuine <a>, and works today only by the ACCIDENT that
 * page.mouse.click's move phase also satisfies :hover before the click lands).
 * Detect structurally + behaviourally, exactly like a click-reveal test but with
 * a MOUSE MOVE instead of a click: find a horizontal ROW or vertical COLUMN of
 * similar-sized items inside a real <nav>/<header>/[role=navigation] landmark
 * (real HTML/ARIA semantics, not a class name), then for each one NOT already a
 * click-trigger, move the mouse there (never click — hover cannot navigate, so
 * this is inherently safer than click exploration) and diff visible text
 * before/after; only a genuine reveal earns the tag. Continues the SAME
 * data-ppd-idx numbering tagTriggers left off at (startIdx) plus a data-ppd-hover
 * marker, so the rest of the pipeline (triggerInfo, gating, ghosts, state capture)
 * needs no separate code path — only HOW the interaction step moves the pointer
 * differs. Deliberately scoped to header/nav landmarks (not every element on the
 * page) to keep cost bounded — a handful of items, not hundreds — and run ONCE at
 * depth 0 only (the header persists across every state; re-testing it at each of
 * dozens of states would be pure waste).
 */
async function findHoverCandidates(page) {
  const json = await page.evaluate(() => {
    const vis = (e) => { const cs = getComputedStyle(e); return cs.display !== 'none' && cs.visibility !== 'hidden' && parseFloat(cs.opacity || '1') >= 0.05; };
    // display:contents makes an element vanish from LAYOUT (zero-size box) while its
    // children render as if directly under its parent — flatten it so this structural
    // walk still sees what's actually rendered (same rationale as canonical-layout.js's
    // scroll-container detection).
    const bigKids = (e) => {
      const out = [];
      for (const ch of e.children) {
        if (!vis(ch)) continue;
        if (getComputedStyle(ch).display === 'contents') { out.push(...bigKids(ch)); continue; }
        const r = ch.getBoundingClientRect();
        if (r.width >= 15 && r.height >= 15) out.push(ch);
      }
      return out;
    };
    ...
    const bestGroup = (its) => {
      ...
      for (const arr of groups.values()) {
        ...
        if (rowOrColumn(arr) && (!best || arr.length > best.length)) best = arr;
      }
      return best;
    };
    // BFS shallowest-first, stop at the FIRST valid group (same reasoning as the
    // delegated-list detector in state-crawler.js: searching the whole subtree and
    // preferring "most items" can wander past a perfectly good shallow match).
    const findList = (root) => {
      const queue = [[root, 0]];
      while (queue.length) {
        const [n, d] = queue.shift();
        if (d > 8) continue;
        const kids = bigKids(n);
        const found = bestGroup(kids);
        if (found) return found;
        for (const ch of kids) {
          if (ch.getBoundingClientRect().width < n.clientWidth * 0.25 && ch.getBoundingClientRect().height < n.clientHeight * 0.25) continue;
          queue.push([ch, d + 1]);
        }
      }
      return null;
    };
    // Group of h's SIMILAR siblings (same class signature) that form a row/column.
    const flatParent = (e) => { let p = e.parentElement; while (p && getComputedStyle(p).display === 'contents') p = p.parentElement; return p; };
    const siblingGroup = (h) => { const p = flatParent(h); if (!p) return null; const sib = bigKids(p).filter((k) => classSig(k) === classSig(h)); if (sib.length < 2) return null; return rowOrColumn(sib) ? sib : null; };

    // Cache COMBINATOR :hover selectors (a :hover that reveals a descendant/sibling —
    // the clean "menu reveals another element" signal; self-hover cosmetics like
    // a:hover{color} are deliberately excluded). Stylesheets are static across states.
    if (!window.__ppdHoverSel) {
      const sels = [];
      for (const sheet of document.styleSheets) {
        let rules; try { rules = sheet.cssRules; } catch { continue; } // cross-origin (CORS) — skip
        if (!rules) continue;
        for (const rule of rules) {
          if (!rule.selectorText || rule.selectorText.indexOf(':hover') < 0) continue;
          for (let sel of rule.selectorText.split(',')) {
            const hi = sel.indexOf(':hover'); if (hi < 0) continue;
            if (/^\s*[>+~ ]\s*\S/.test(sel.slice(hi + 6))) { const hp = sel.slice(0, hi).trim(); if (hp) sels.push(hp); }
          }
        }
      }
      window.__ppdHoverSel = [...new Set(sels)];
    }

    // H = hover-behaviour elements: combinator :hover targets ∪ JS hover-listeners
    // (instrument), currently VISIBLE. General — anywhere on the page, not nav/header.
    const H = new Set();
    for (const hp of window.__ppdHoverSel) { try { for (const el of document.querySelectorAll(hp)) if (vis(el)) H.add(el); } catch {} }
    for (const el of document.querySelectorAll('[data-ppd-hoverlistener]')) if (vis(el)) H.add(el);

    // INTERSECT with list SHAPE. A hover element contributes EITHER the row/column of
    // similar siblings it's itself part of (per-item listeners, e.g. idfc's <li>s), OR
    // — if it isn't a list item — the item list found INSIDE it (a DELEGATED listener
    // on a container like grace's short-but-wide <header>, whose nav row lives within).
    // A lone hover element that's neither is skipped — that structural gate is the cost
    // control: an open mega-panel's ~100 loose hover-links don't each become a
    // candidate, only genuine menu/tab lists do.
    const cand = new Set();
    for (const h of H) {
      const g = siblingGroup(h);
      if (g) { for (const it of g) cand.add(it); continue; }
      const items = findList(h);
      if (items) for (const it of items) if (vis(it)) cand.add(it);
    }
    // Drop click-covered, keep only LEAVES (hovering an inner item covers its ancestor).
    let list = [...cand].filter((e) => vis(e) && !e.hasAttribute('data-ppd-idx'));
    list = list.filter((e) => !list.some((o) => o !== e && e.contains(o)));
    list = list.slice(0, 80);
    const out = [];
    let uid = 0;
    for (const it of list) { const r = it.getBoundingClientRect(); const tag = `hc${uid++}`; it.setAttribute('data-ppd-hover-cand', tag); out.push({ tag, x: Math.round(r.left + r.width / 2), y: Math.round(r.top + r.height / 2) }); }
...
/**
 * Behavioural pass: for each structural hover-CANDIDATE, real mouse-move (never
 * click), diff visible text before/after, and tag only genuine reveals with the
 * SAME data-ppd-idx numbering as click-triggers (continuing from startIdx) plus
 * data-ppd-hover. Moves the mouse away between candidates so one test's reveal
 * doesn't bleed into the next's "before" snapshot. Returns the next free index.
 */
async function tagHoverTriggers(page, startIdx, hoverAnchor = null) {
  const cands = await findHoverCandidates(page);
  const back = hoverAnchor || { x: 2, y: 2 };
  let idx = startIdx;
  for (const cand of cands) {
    const handle = await page.$(`[data-ppd-hover-cand="${cand.tag}"]`);
    if (!handle) continue;
    try {
      const before = await harvestVisibleTextInPage(page);
      const beforeSet = new Set(before.map((l) => l.t.toLowerCase()));
      await page.mouse.move(cand.x, cand.y);
      await page.waitForTimeout(300);
      const after = await harvestVisibleTextInPage(page);
      const added = after.filter((l) => !beforeSet.has(l.t.toLowerCase()));
      // Return to the ANCESTOR hover anchor (keeps an open parent panel open so nested
      // candidates stay testable); at depth 0 that's the corner (nothing to preserve).
      await page.mouse.move(back.x, back.y);
      await page.waitForTimeout(120);
      if (added.length > 0) {
        await handle.evaluate(
          (el, i) => { el.setAttribute('data-ppd-idx', String(i)); el.setAttribute('data-ppd-hover', '1'); },
          idx
        );
        idx += 1;
      }
    } catch { /* best-effort — a candidate that errors is simply skipped */ }
    try { await handle.evaluate((el) => el.removeAttribute('data-ppd-hover-cand')); } catch { /* ignore */ }
  }
  return idx;
}
...
async function exploreStates(page, opts = {}) {
  ...
  const captureState = async (info, addedLines, sig, parentSig, depth) => {
    ...
    let childRects = null;
    let revealText = null;
    // After a click that expands an accordion / grows content, the layout AND window.scrollY
    // keep drifting for up to ~1s (accordion settle + the browser's scroll-anchoring silently
    // re-adjusting scrollY). If we record captureScrollY before that settles, the shot and the
    // child/reveal coords end up in DIFFERENT scroll frames → boxes land ~50-300px off. Wait
    // for scrollY to stabilise (and turn OFF scroll-anchoring so it stops auto-drifting) so the
    // shot, captureScrollY and the geometry snapshot all share one frame.
    const settleScroll = () =>
      page.evaluate(async () => {
        ...
        for (let i = 0; i < 12; i += 1) {
          ...
          if (y === last) { stable += 1; if (stable >= 2) return y; } else { stable = 0; }
          last = y;
    ...
    if (captureStates) {
      ...
      if (box && !box.fixed) {
        ...
        if (vh > 0 && regionH > vh) {
          ...
          try {
            ...
            vpW = scr.vw;
            vpH = scr.vh;
            const settled = await settleScroll().catch(() => null); // wait out accordion/anchor drift
            if (settled != null) captureScrollY = settled; // record the FRAME the shot is taken in
            await withClm();
      ...
      if (!done) {
        ...
        try {
          ...
          vpW = scr.vw;
          vpH = scr.vh;
          const settled = await settleScroll().catch(() => null);
          if (settled != null) captureScrollY = settled;
        } catch {
      ...
      // Snapshot clickable-trigger doc rects WHILE STILL SCROLLED at captureScrollY —
      // i.e. in the SAME scroll regime as this screenshot. A sticky header shifts
      // absolute document Y by its height when it pins, so a child trigger measured
      // later at a different scroll (e.g. scroll-0 after a reset) records a Y ~53px off
      // this shot. buildGraph corrects each child box's Y from this snapshot, matched by
      // the SCROLL-INVARIANT x/w/h, so boxes sit on the card as the shot shows it.
      try {
        childRects = JSON.parse(
          await page.evaluate(() => {
            const out = [];
            for (const el of document.querySelectorAll('[data-ppd-idx],[data-ppd-click],a[href],button,summary,[role="button"],[role="tab"],[role="menuitem"]')) {
              const r = el.getBoundingClientRect();
              if (r.width < 6 || r.height < 6) continue;
              out.push([Math.round(r.left + window.scrollX), Math.round(r.top + window.scrollY), Math.round(r.width), Math.round(r.height)]);
            }
            return JSON.stringify(out);
          })
        );
      ...
      // Per-line rects of this state's NEWLY-REVEALED text, captured in the SAME scroll
      // regime as the shot (so highlights land on the words). Matched by normalised text
      // to addedLines; a band around the trigger drops far-away duplicates (nav/EDS
      // repeats). buildGraph tags each line present/missing on the other side for the
      // green(source-only)/red(target-only)/blue(both) highlight.
      try {
        const wantNorms = (addedLines || []).map(norm).filter((x) => x && x.length >= 6);
        if (wantNorms.length) {
          revealText = JSON.parse(
            await page.evaluate(({ norms, tY }) => {
              if (!document.body) return JSON.stringify([]);
              const nf = (s) => String(s || '').toLowerCase().replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();
              const want = new Set(norms);
              const out = [];
              const seen = new Set();
              const wk = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT);
              for (let n = wk.nextNode(); n && out.length < 250; n = wk.nextNode()) {
                const t = nf(n.nodeValue || '');
                if (t.length < 6 || !want.has(t)) continue;
                const el = n.parentElement;
                if (!el || el.closest('script,style,template,noscript')) continue;
                const cs = getComputedStyle(el);
                ...
                let r;
                try { const rg = document.createRange(); rg.selectNodeContents(n); r = rg.getBoundingClientRect(); } catch { continue; }
                if (r.width < 2 || r.height < 2) continue;
                const y = Math.round(r.top + (window.scrollY || 0));
                if (tY != null && y < tY - 800) continue; // reveal is at/below the trigger
                const key = Math.round(r.left / 6) + ',' + Math.round(y / 6);
                if (seen.has(key)) continue;
                seen.add(key);
                out.push([Math.round(r.left + (window.scrollX || 0)), y, Math.round(r.width), Math.round(r.height), t.slice(0, 80)]);
              }
              return JSON.stringify(out);
            }, { norms: wantNorms, tY: info.y == null ? null : info.y })
          );
        }
      } catch {
        /* best-effort */
    ...
    states.push({
      ...
      viewportWidth: vpW,
      viewportHeight: vpH,
      childRects, // [[x,y,w,h],...] doc rects at this shot's scroll regime (for box-Y correction)
      revealText, // [[x,y,w,h,normText],...] newly-revealed text lines (same scroll regime)
    });
  };
   ...
   */
  // `hoverAnchor` = {x,y} of the OPEN parent hover trigger whose panel this state
  // lives inside (null at base / for DOM-driven click states). Lets a sibling's hover
  // panel be closed WITHOUT closing the ancestor's (move back to the anchor, not the
  // corner), so nested hover menus stay reachable across the sibling sweep.
  const explore = async (curStable, curSig, depth, parentKeys = new Set(), hoverAnchor = null) => {
    ...
    const clickTagCount = await timed('tag', () => tagTriggers(page, TRIGGER_SEL, ITEM_SEL, CLICK_DEBUG));
    // Hover-reveal triggers at depth 0 AND depth 1 (not just the header). Grace-style
    // menus open a mega-panel on hover (found at depth 0) and its vertical tabs on a
    // further hover (found at depth 1). Capping at depth 1 bounds cost — it covers the
    // common menu→panel→tabs while stopping the interaction blow-up a large mega-menu
    // causes if hover is probed at every depth. Candidates are hover-behaviour ∩ list-
    // shape (see findHoverCandidates), so only genuine menu/tab lists are tested, and
    // the behavioural pre-filter tags only real reveals. Same data-ppd-idx numbering.
    if (depth <= 1) {
      try {
        const hoverTagCount = await timed('hoverTag', () => tagHoverTriggers(page, clickTagCount, hoverAnchor));
        if (CLICK_DEBUG && hoverTagCount > clickTagCount) {
          const hovered = JSON.parse(
            await page.evaluate(() =>
              JSON.stringify(
                [...document.querySelectorAll('[data-ppd-hover]')].map((el) => ({
                  idx: el.getAttribute('data-ppd-idx'),
                  label: (el.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 40),
                }))
              )
            )
          );
          clog(`d0 hoverTag: ${hovered.length} hover-reveal triggers tagged`);
          for (const h of hovered) clog(`  HOVER#${h.idx} "${h.label}"`);
        }
      } catch (e) {
        clog('hover-tag failed', e && e.message);
      }
    }
    if (CLICK_DEBUG) {
      try {
    ...
    // A hover-revealed panel is CSS :hover-driven — it closes the instant the cursor
    // leaves, unlike a click-revealed state (JS/DOM-driven, reversed by the mutation-
    // undo stack regardless of cursor position). So after fully processing a hover
    // trigger's subtree, the mouse must be moved away before the NEXT sibling trigger
    // starts, or its "before" state is still the previous panel's open state. Deferred
    // to the top of the next iteration (and once after the loop) rather than at every
    // one of this loop's several early continue/return points below.
    let pendingHoverClose = false;
    const closeHoverPanel = async () => {
      if (!pendingHoverClose) return;
      pendingHoverClose = false;
      // Move to the ANCESTOR hover anchor (keeps the parent panel open so this level's
      // remaining siblings stay visible); only at the top level go to the corner.
      const to = hoverAnchor || { x: 2, y: 2 };
      try { await page.mouse.move(to.x, to.y); await page.waitForTimeout(150); } catch { /* best-effort */ }
    };

    for (let i = 0; i < limit; i++) {
      await closeHoverPanel();
      let hoverPos = null; // {x,y} if THIS iteration hovered a trigger (child anchor)
      ...
      if (!info) continue;
      // A hover-reveal trigger (tagged by tagHoverTriggers, depth 0 only): the
      // interaction step below moves the mouse instead of clicking — never click a
      // hover-only element, since a real click could navigate on a genuine <a> that
      // hover never would (hover cannot navigate at all — that's what makes it the
      // safe/correct model for a header mega-menu item).
      let isHover = false;
      try { isHover = await handle.evaluate((el) => el.hasAttribute('data-ppd-hover')); } catch { /* default false */ }
      ...
      const clippedAtBase = !!info.clipped; // scroll-0 horizontal clip → truly off the backdrop
      ...
      if (CLICK_DEBUG) {
        ...
        clog(`d${depth} #${i} ${isHover ? 'HOVER' : 'CLICK'} <${m && m.tag}${m && m.ppd ? ' ppd' : ''}> "${(info.label || '').slice(0, 40)}" href=${m && m.href} @${info.x},${info.y} ${info.w}x${info.h} .${m && m.cls}`);
      }
      ...
      // Per-click DRIFT reference: a full-page visible-text snapshot taken NOW, just
      // before the click, so the change gate diffs against what was ACTUALLY on
      // screen at click time — not the state's (possibly stale) entry snapshot
      // (curStable). A background region that some earlier interaction knocked off
      // its frozen state (a hero carousel bumped to another slide, then persisting)
      // sits in BOTH this snapshot and the after-state, so it can no longer
      // masquerade as an unrelated click's reveal — the actual cause of the
      // "every late trigger points to the same shifted hero text" false positives.
      // Cheap (~13ms) and scroll-independent (visibility-based, not viewport-based),
      // so the scroll-to-centre above doesn't affect it. The occlusion/pixel second
      // opinions already diff per-click (occBefore); this brings the text gate in line.
      let clickBeforeSet = null;
      try {
        const cb = await timed('clickBefore', () => harvestVisibleTextInPage(page));
        clickBeforeSet = new Set();
        for (const o of cb) { const k = norm(o.t); if (k.length >= 2) clickBeforeSet.add(k); }
      } catch {
        /* fall back to the per-state reference (curStable) below */
      }
      ...
      await timed('click', async () => {
        ...
        // Not in the backdrop → the review needs a box + ghost. The backdrop is a
        // full-HEIGHT scroll-0 screenshot, so "not in it" is a HORIZONTAL/scroll-0 fact:
        // the trigger was CLIPPED out horizontally at scroll 0 (clippedAtBase, an off-
        // screen carousel/strip card), OR its RECORDED box runs past the backdrop width
        // (x<0 or x+w>viewport width). Do NOT use the click-time occlusion gate
        // (gateCode 1) here: that fires when a trigger is merely covered at the instant
        // of clicking (mid-scroll a decorative figure overlapped the visible Key Benefits
        // cards) even though it's plainly visible in the scroll-0 backdrop — which wrongly
        // ghosted visible cards. gateCode 1 still drives the un-clip for the CLICK below.
        if (clippedAtBase || gateCode === 2 || geom.triggerX < 0 || geom.triggerX + geom.triggerW > vp.width) geom.hidden = true;
        ...
        // CAROUSEL-ADVANCE: a horizontally-off trigger (gateCode 2) is an off-screen
        // carousel slide — a coordinate click would miss it and el.click() NOOPs (the
        // slide's action is a delegated pointer handler that only fires on a trusted
        // click at the real pointer location). Scroll its nearest horizontally-scrollable
        // ancestor (scrollWidth > clientWidth — a native strip OR a transform-swiper,
        // which also overflows) so the slide enters the viewport; the trusted click below
        // then fires its real handler. Validated end-to-end (brings every off-screen slide
        // on-screen with no snap-back, opens the slide's modal). Done AFTER the ghost
        // capture (which scrolls + de-transforms and would clobber the advance) and
        // RESTORED after the click so backtracking / the next trigger start clean. Only
        // fires for a genuine MISS (off-screen), so it never disturbs a visible swiper.
        if (gateCode === 2) {
          try {
            await handle.evaluate((el) => {
              window.__ppdCarousel = null;
              const r = el.getBoundingClientRect();
              for (let p = el.parentElement; p; p = p.parentElement) {
                if (!/(auto|scroll|hidden)/.test(getComputedStyle(p).overflowX)) continue;
                if (p.scrollWidth <= p.clientWidth + 2) continue;
                const pr = p.getBoundingClientRect();
                const prev = p.scrollLeft;
                // bring the slide's left just inside the container's left edge (works for
                // slides off to EITHER side — delta is signed)
                const want = Math.max(0, Math.min(p.scrollWidth - p.clientWidth, prev + (r.left - pr.left) - 12));
                p.scrollLeft = want;
                window.__ppdCarousel = [p, prev];
                break;
              }
            });
            await page.waitForTimeout(150); // let the scroll settle before re-measuring
          } catch { /* best-effort */ }
        }
        let box = null;
        ...
        if (box && box.width >= 1 && box.height >= 1) {
          ...
          if (cx >= 0 && cy >= 0 && cx <= vp.width && cy <= vp.height) {
            try {
              // HOVER: real mouse move only, never a click — a click could navigate
              // on a genuine <a> that hover never would.
              if (isHover) { await page.mouse.move(cx, cy); pendingHoverClose = true; hoverPos = { x: cx, y: cy }; }
              else await page.mouse.click(cx, cy);
              clicked = true;
            } catch { /* fall back */ }
          }
        }
        // No synthetic-hover fallback: dispatching a JS mouseover/mouseenter event
        // does NOT reliably trigger CSS :hover matching (that's cursor-position-
        // driven at the engine level, not event-driven) — el.click() is only a valid
        // fallback for an actual click.
        if (!clicked && !isHover) {
        ...
        try {
          await page.evaluate(() => {
            ...
            window.__ppdUnclip = null;
            // Undo any carousel-advance so the base is clean for backtracking / next trigger.
            if (window.__ppdCarousel) { const [p, prev] = window.__ppdCarousel; try { p.scrollLeft = prev; } catch { /* gone */ } window.__ppdCarousel = null; }
          });
        } catch { /* best-effort */ }
      });
      ...
      try {
        ...
        // Compare against what was on screen just before THIS click (drift-robust),
        // not the state's entry snapshot; fall back to curStable if that failed.
        const parentSet = clickBeforeSet || new Set(curStable);
      ...
      // DRIFT FILTER (see clickBeforeSet): attribute a delta to THIS click only if it
      // actually changed AT click time. `added` keeps only lines that were NOT on
      // screen just before the click (a genuine reveal never is); `removed` keeps only
      // lines that WERE (a real removal was). A persistent background drift — a line
      // already visible pre-click, or a base line that drift had already replaced —
      // is excluded from both. Both use clickBeforeSet purely as a FILTER over the
      // sampled delta, never as a source, so no single-harvest volatile noise leaks
      // in. Falls back to the raw delta if the pre-click snapshot failed.
      const curStableSet = new Set(curStable);
      const addedGenuine = clickBeforeSet ? edge.added.filter((l) => !clickBeforeSet.has(l)) : edge.added;
      const removedGenuine = clickBeforeSet ? edge.removed.filter((l) => clickBeforeSet.has(l)) : edge.removed;
      // For state IDENTITY/dedup and the recursion base, strip drift lines (visible
      // pre-click yet not part of this state's base) from the after-set, so the SAME
      // state reached at different drift points still dedups to one.
      const childStableClean = clickBeforeSet
        ? childStable.filter((l) => !(clickBeforeSet.has(l) && !curStableSet.has(l)))
        : childStable;
      ...
      if (addedGenuine.length < minAdded && removedGenuine.length < minAdded) {
      ...
      const childDelta = deltaFromBase(childStableClean);
      const known = findVisited(childDelta, visited, matchOpts);
      if (known) {
        clog(`  -> KNOWN state (text delta matched a visited state; added ${JSON.stringify(addedGenuine.slice(0, 2))})`);
        pushEdge(known.key.hash, 'known');
        await page.evaluate(() => window.__ppdMutUndo());
        await resetScroll();
        continue;
      ...
      const childSig = signatureKey(childDelta);
      visited.push({ key: childSig, stable: childDelta });
      await timed('captureState', () => captureState(info, addedGenuine, childSig, curSig, depth + 1));
      clog(`  -> NEW state +${addedGenuine.length} (added ${JSON.stringify(addedGenuine.slice(0, 3))})`);
      pushEdge(childSig.hash, 'new');

      // A hover-revealed panel closes when the cursor leaves, so for a hover child we
      // must NOT scroll away — re-assert the hover so the panel stays open while its
      // nested triggers are explored, and make its hoverPos the child's anchor. A click
      // child is DOM-driven → scroll-reset as before and inherit our ancestor anchor.
      if (hoverPos) { try { await page.mouse.move(hoverPos.x, hoverPos.y); await page.waitForTimeout(120); } catch { /* best-effort */ } }
      else await resetScroll(); // recurse from scroll 0 (the pixel before-frame scrolled us)
      await explore(childStableClean, childSig, depth + 1, childParentKeys, hoverPos || hoverAnchor);
    ...
    await closeHoverPanel(); // the last trigger processed may have been a hover-reveal
  };
