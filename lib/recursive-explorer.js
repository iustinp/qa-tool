...
async function findHoverCandidates(page) {
  const json = await page.evaluate(() => {
    ...
    const isContainer = (e) => { const r = e.getBoundingClientRect(); return r.width > vw * 0.6 && r.height > 200; };
    ...
    // INTERSECT with list SHAPE. Each hover element contributes the row/column of
    // similar siblings it belongs to (or, if it's a big DELEGATED container like a
    // <header>, the item list inside it). A lone hover element that isn't part of a
    // list is skipped — that structural gate is the cost control: an open mega-panel's
    // ~100 loose hover-links don't each become a candidate, only genuine menu/tab lists.
    const cand = new Set();
    for (const h of H) {
      if (isContainer(h)) { const items = findList(h); if (items) for (const it of items) if (vis(it)) cand.add(it); }
      else { const g = siblingGroup(h); if (g) for (const it of g) cand.add(it); }
    }
