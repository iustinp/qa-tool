...
async function findHoverCandidates(page) {
  const json = await page.evaluate(() => {
    ...
    let uid = 0;
    for (const root of document.querySelectorAll('nav, header, [role=navigation]')) {
      const items = findList(root);
      if (!items || seenRoots.has(items[0])) continue;
      seenRoots.add(items[0]);
      for (const it of items) {
        if (it.hasAttribute('data-ppd-idx')) continue; // already a click candidate — click already covers it
        const r = it.getBoundingClientRect();
        const tag = `hc${uid}`;
        uid += 1;
        it.setAttribute('data-ppd-hover-cand', tag);
        out.push({ tag, x: Math.round(r.left + r.width / 2), y: Math.round(r.top + r.height / 2) });
      }
    }
 ...
 */
async function tagHoverTriggers(page, startIdx) {
  ...
  for (const cand of cands) {
    ...
    try {
      ...
      await page.mouse.move(2, 2); // move away — close the panel before the next candidate
...
async function exploreStates(page, opts = {}) {
  ...
  const explore = async (curStable, curSig, depth, parentKeys = new Set()) => {
    ...
    // Hover-reveal triggers (header mega-menus): the header persists across every
    // state, so test it ONCE at depth 0 rather than re-testing at each of dozens of
    // states — pure waste otherwise. Continues the SAME data-ppd-idx numbering
    // click-triggers left off at, so the rest of the pipeline needs no separate path.
    if (depth === 0) {
      try {
        const hoverTagCount = await timed('hoverTag', () => tagHoverTriggers(page, clickTagCount));
    ...
    const closeHoverPanel = async () => {
      ...
      try { await page.mouse.move(2, 2); await page.waitForTimeout(150); } catch { /* best-effort */ }
    };
    ...
    for (let i = 0; i < limit; i++) {
        ...
        if (box && box.width >= 1 && box.height >= 1) {
          ...
          if (cx >= 0 && cy >= 0 && cx <= vp.width && cy <= vp.height) {
            try {
              ...
              if (isHover) { await page.mouse.move(cx, cy); pendingHoverClose = true; }
      ...
      await explore(childStableClean, childSig, depth + 1, childParentKeys);
