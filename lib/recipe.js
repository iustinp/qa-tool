...
const DEFAULT_RECIPE = {
  ...
  profiles: ['desktop'],
  resolutions: [], // [{ width, ua: 'desktop'|'mobile', height? }] — analyze at these widths (overrides `profiles`)
  ignore: [], // [{ selector, reason }] — removed from BOTH sides (shared chrome: cookie banners)
  ignoreSource: [], // [{ selector, reason }] — marked/hatched on the SOURCE, excluded from compare
  ignoreTarget: [], // [{ selector, reason }] — marked/hatched on the TARGET, excluded from compare
  clickSource: [], //  [{ selector, reason }] — clicked if present on SOURCE (e.g. cookie accept)
  clickTarget: [], //  [{ selector, reason }] — clicked if present on TARGET
...
function withDefaults(partial) {
  ...
  return {
    ...
    ignoreSource: Array.isArray(p.ignoreSource) ? p.ignoreSource : [],
    ignoreTarget: Array.isArray(p.ignoreTarget) ? p.ignoreTarget : [],
    clickSource: Array.isArray(p.clickSource) ? p.clickSource : [],
    clickTarget: Array.isArray(p.clickTarget) ? p.clickTarget : [],
...
function validateRecipe(recipe) {
  ...
  recipe.resolutions.forEach((r, i) => {
    const w = Number(r && r.width);
    if (!Number.isFinite(w) || w < 100 || w > 4000) {
      errors.push(`resolutions[${i}] needs a numeric "width" between 100 and 4000`);
    }
    if (r && r.ua != null && r.ua !== 'desktop' && r.ua !== 'mobile') {
      warnings.push(`resolutions[${i}].ua "${r.ua}" unknown (use desktop|mobile) — treating as desktop`);
    }
  });
  for (const key of ['ignore', 'ignoreSource', 'ignoreTarget', 'clickSource', 'clickTarget']) {
    recipe[key].forEach((r, i) => {
      if (!r || typeof r.selector !== 'string' || !r.selector.trim()) {
        errors.push(`${key}[${i}] needs a non-empty "selector"`);
      }
    });
  }
