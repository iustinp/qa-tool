 ...
 */
/**
 * Blank already-matched regions out of the target *in place* (paint white),
 * preserving full dimensions. Unlike stitch-removal this keeps every coordinate
 * and the page height stable, so matched content is visually gone (anti-drift)
 * without breaking full-page bbox math or relative-position reasoning.
 * @param {Buffer} fullBuffer full target PNG
 * @param {Array<{bbox:{x,y,width,height}}>} regions matched regions (full-page px)
 */
async function blankMatchedRegions(fullBuffer, regions) {
  if (!regions || regions.length === 0) return Buffer.from(fullBuffer);
  const meta = await sharp(fullBuffer).metadata();
  const imgW = meta.width || 1;
  const imgH = meta.height || 1;
  const overlays = [];
  for (const region of regions) {
    const r = clampRect(region.bbox, imgW, imgH);
    ...
    const white = await sharp({
      ...
      .png()
      .toBuffer();
    overlays.push({ input: white, top: r.y, left: r.x });
  }
  if (overlays.length === 0) return Buffer.from(fullBuffer);
  return sharp(fullBuffer).composite(overlays).png().toBuffer();
}
