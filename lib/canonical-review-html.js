...
function box(it,color,side){ const g=document.createElementNS(SVGNS,'g');
  ...
  r.setAttribute('pointer-events','all'); r.style.cursor='pointer'; r.onclick=()=>syncTo(it,side); g.appendChild(r);
...
function syncTo(it,side){
  if(it.cx==null){ syncShift=null; syncAnchor=null; el('bReset').style.display='none'; render(); return; }
  const other=side==='source'?'target':'source';
  syncShift={side:other,dx:it.x-it.cx,dy:it.y-it.cy}; syncAnchor={side,i:it.i};
  el('bReset').style.display='inline-block'; render();
}
...
el('bReset').onclick=()=>{ syncShift=null; syncAnchor=null; el('bReset').style.display='none'; render(); };
