...
function applyView(){
  ...
  // The screenshot is displayed at its TRUE document size (natural px / dpr, set in
  // sizeShot on load), which can exceed the extraction-time *Dims when the page's
  // document width differs from it (horizontal overflow) — grow the canvas to fit it
  // so the shot isn't shrunk/compressed and its content registers 1:1 with the pears.
  { const sc=shotCss[bgSide]; if(sc){ cw=Math.max(cw,sc.w); ch=Math.max(ch,sc.h); } }
...
// One drift per inline paragraph (bid b), measured at its first run in reading order vs
// the dense pool of runs OUTSIDE the paragraph (the local page shift). Every run inherits
// it so a group's connectors share one colour. Mirror of driftBuckets in
// text-layout-compare.js.
function computeDrifts(){
  const M=D.source.filter(it=>it.m&&it.cx!=null);
  M.forEach((it,i)=>{ it._bk=(it.b!=null?'b'+it.b:'s'+i); });
  const anchor={};
  for(const it of M){ const a=anchor[it._bk];
    if(!a||it.y<a.y||(it.y===a.y&&it.x<a.x)) anchor[it._bk]=it; }
  const driftByKey={};
  for(const k in anchor){ const it=anchor[k];
    const near=M.filter(o=>o._bk!==k).sort((a,b)=>Math.abs(a.y-it.y)-Math.abs(b.y-it.y)).slice(0,DRIFT_K);
    if(!near.length){ driftByKey[k]=0; continue; }
    const mdx=median(near.map(o=>o.cx-o.x)), mdy=median(near.map(o=>o.cy-o.y));
    driftByKey[k]=Math.max(Math.abs((it.cx-it.x)-mdx),Math.abs((it.cy-it.y)-mdy));
  }
  for(const it of M){ it._drift=driftByKey[it._bk]||0; } }
...
function fillSide(){ const s=el('side'); const txt=i=>!i.img;
  ...
  el('stat').innerHTML='<small>'
    ...
    +(a.driftScore!=null?' · <span title="positional drift of matched elements — LOWER is better, 0 = none (red-dominant)">Drifting Items '+a.driftScore+'%</span> <span style="opacity:.65">('+g(a.driftGreen)+'/'+y(a.driftYellow)+'/'+r(a.driftRed)+')</span>':'')
