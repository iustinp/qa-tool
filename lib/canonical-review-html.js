...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .row.gated{color:#7a9a7a;cursor:default}.row.gatedmiss{color:#a97a7a;cursor:default}
...
function render(){ const ov=el('ov'); ov.innerHTML=''; const showAlign=el('tAlign').checked; const showMoved=el('tMoved').checked;
  ...
  const trigs=D.triggers&&D.triggers[bgSide]; // crawl triggers for this side (interactive)
  // Passive clickable markers (purple dashed) only when there's no crawl data —
  // drawn under the boxes. With crawl data we draw interactive triggers on top.
  if(el('tClick').checked && !(trigs&&trigs.length)){ const o=off(bgSide); for(const c of (D.clickables[bgSide]||[])){ const r=document.createElementNS(SVGNS,'rect');
    r.setAttribute('x',c.x+o.dx);r.setAttribute('y',c.y+o.dy);r.setAttribute('width',c.w);r.setAttribute('height',c.h);
    r.setAttribute('fill','none');r.setAttribute('stroke','var(--click)');r.setAttribute('stroke-width','1.5');r.setAttribute('stroke-dasharray','2 2');r.setAttribute('opacity','0.7'); ov.appendChild(r); } }
  ...
  if(el('tClick').checked && trigs){ const o=off(bgSide); for(const t of trigs){ const r=document.createElementNS(SVGNS,'rect');
    r.setAttribute('x',t.x+o.dx);r.setAttribute('y',t.y+o.dy);r.setAttribute('width',t.w);r.setAttribute('height',t.h);
    r.setAttribute('fill','var(--click)');r.setAttribute('fill-opacity','0.06');r.setAttribute('stroke','var(--click)');r.setAttribute('stroke-width','1.5');
    r.setAttribute('pointer-events','all'); r.style.cursor='pointer'; r.onclick=(e)=>{ e.stopPropagation(); showReveal(t); }; ov.appendChild(r); } }
...
function fillSide(){ const s=el('side'); const txt=i=>!i.img;
  ...
  if(D.gated){
    const grow=(arr,cls)=>arr.map(i=>'<div class="row '+cls+'">'+escapeHtml(i.text.slice(0,70))+(i.trigger?' <small>via “'+escapeHtml(i.trigger.slice(0,26))+'”</small>':'')+'</div>').join('');
    s.innerHTML+='<h4 class="gatedh" title="on target only after an interaction">Interaction-gated, present ('+D.gated.present.length+')</h4>'+grow(D.gated.present,'gated')
      +'<h4 class="gatedh" title="source reveals via interaction; target never shows it">Interaction-gated, missing ('+D.gated.missing.length+')</h4>'+grow(D.gated.missing,'gatedmiss');
  }
