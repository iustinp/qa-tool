...
function applyView(){
  ...
  // Hidden slider items flow off the right edge (slides 2..N) — widen the canvas so
  // the stage can scroll to reach them when 'Hidden content' is on (#63).
  if(el('tHidden')&&el('tHidden').checked&&D.hidden&&D.hidden[bgSide]){ for(const it of (D.hidden[bgSide].items||[])){ cw=Math.max(cw, it.x+it.w+8); ch=Math.max(ch, it.y+it.h+8); } }
  el('inner').style.width=cw+'px'; el('inner').style.height=ch+'px';
  const ov=el('ov'); ov.setAttribute('width',cw); ov.setAttribute('height',ch); ov.setAttribute('viewBox','0 0 '+cw+' '+ch);
...
function render(){ if(navStack.length){ renderStateOverlay(); return; } const ov=el('ov'); ov.innerHTML=''; const showAlign=el('tAlign').checked; const showMoved=el('tMoved').checked;
  ...
  if(el('tHidden').checked && D.hidden && D.hidden[bgSide]){ const o=off(bgSide); const H=D.hidden[bgSide];
    ...
    for(const it of (H.items||[])){ const r=document.createElementNS(SVGNS,'rect');
      ...
      r.setAttribute('fill','var(--hidden)');r.setAttribute('fill-opacity','0.05');r.setAttribute('stroke','var(--hidden)');r.setAttribute('stroke-width','1');r.setAttribute('stroke-dasharray','4 3');r.setAttribute('opacity','0.85'); ov.appendChild(r); }
    for(const n of (H.nodes||[])){ const flagged=flagSet.has(n.text);
      const t=document.createElementNS(SVGNS,'text');
      t.setAttribute('x',n.x+o.dx);t.setAttribute('y',n.y+o.dy+Math.min(n.h,15)*0.82);
      t.setAttribute('font-size',Math.max(9,Math.min(n.h,15)));t.setAttribute('font-family','system-ui,sans-serif');
      t.setAttribute('fill',flagged?flagCol:'#5a6b7a');t.setAttribute('opacity',flagged?'0.95':'0.55');
      t.textContent=(n.text||'').slice(0,64); ov.appendChild(t);
      if(flagged){ const r=document.createElementNS(SVGNS,'rect');
        ...
        r.setAttribute('fill','none');r.setAttribute('stroke',flagCol);r.setAttribute('stroke-width','1.5'); ov.appendChild(r); } }
  }
...
// re-runs applyView (which recomputes width) rather than just render.
el('tHidden').onchange=applyView;
