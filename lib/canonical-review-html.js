...
function render(){ if(navStack.length){ renderStateOverlay(); return; } const ov=el('ov'); ov.innerHTML=''; const showAlign=el('tAlign').checked; const showMoved=el('tMoved').checked;
  ...
  // Interactive layer (on top). SOLID descendable boxes come straight from the
  // state GRAPH base children — EVERY state reachable from base, including the
  // text-less reorder/layout states from the position-delta signal (which the old
  // text-reveal list had no entry for). Clicking descends into the state.
  if(el('tClick').checked){ const o=off(bgSide);
    const g=D.graph&&D.graph[bgSide];
    for(const c of ((g&&g.baseChildren)||[])){ const r=document.createElementNS(SVGNS,'rect');
      r.setAttribute('x',c.x+o.dx);r.setAttribute('y',c.y+o.dy);r.setAttribute('width',c.w);r.setAttribute('height',c.h);
      ...
      r.setAttribute('pointer-events','all'); r.style.cursor='pointer'; r.onclick=(e)=>{ e.stopPropagation(); enterStateSig(bgSide,c.sig); }; ov.appendChild(r); }
    // Weak, informational: text-reveal triggers that did NOT open a captured
    // state — still pop up what they expose vs the other side.
    if(trigs) for(const t of trigs){ if(t.sig) continue; const r=document.createElementNS(SVGNS,'rect');
...
function showState(){
  ...
  // Preserve the scroll position: the state backdrop is the same page + the
  // reveal, so staying where the user clicked keeps context (no jump to top).
  renderStateOverlay();
}
