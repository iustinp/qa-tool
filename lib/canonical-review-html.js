...
function box(it,color,side){ const g=document.createElementNS(SVGNS,'g');
  ...
  return g; }
// The connector line still runs source-origin → target-origin (its length shows
// the absolute shift), but its COLOUR grades RELATIVE drift: how much this
// item's shift differs from the previous matched item's shift. A uniformly
// shifted page (harmless cumulative drift) scores ~0 and stays green; only a
// genuinely-misplaced element spikes. max over x and y, thresholds 20/40:
//   <=20 green (OK) · <=40 yellow (ok-ish) · >40 red (needs work).
// Moved (reordered) items stay orange — a different signal than local drift.
const CONN_GOOD=20, CONN_OKISH=40;
// Precompute each matched source item's relative drift vs the previous matched
// item (skipping missing/moved), mirroring the audit's layoutDrift signal.
function computeDrifts(){ let prev=null;
  for(const it of D.source){ if(it.mv||!it.m||it.cx==null) continue;
    if(prev){ const dx=Math.abs((it.cx-prev.cx)-(it.x-prev.x)); const dy=Math.abs((it.cy-prev.cy)-(it.y-prev.y)); it._drift=Math.max(dx,dy); }
    else it._drift=0;
    prev=it; } }
