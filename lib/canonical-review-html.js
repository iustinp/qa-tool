...
function applyView(){
  ...
  const g=D.graph&&D.graph[bgSide]; let cw=d.w;
  if(g&&g.baseChildren) for(const c of g.baseChildren) cw=Math.max(cw, c.x+c.w+4);
  el('inner').style.width=cw+'px'; el('inner').style.height=d.h+'px';
  const ov=el('ov'); ov.setAttribute('width',cw); ov.setAttribute('height',d.h); ov.setAttribute('viewBox','0 0 '+cw+' '+d.h);
