...
function fillSide(){ const s=el('side'); const txt=i=>!i.img;
  ...
  el('stat').innerHTML='<small>'
    +(a.globalScore!=null?'<b title="global health, 0=identical, higher=worse (0.7·content + 0.3·drift)">health '+a.globalScore+'</b>':'')
    +(a.contentScore!=null?' · <span title="content completeness, missing-dominant">content '+a.contentScore+'</span> <span style="opacity:.65">('+(a.matchedCount!=null?a.matchedCount:'?')+'✓ '+r(a.missingCount)+' miss '+y(a.extraCount)+' extra)</span>':'')
    +(a.driftScore!=null?' · <span title="positional drift of matched elements, red-dominant">drift '+a.driftScore+'</span> <span style="opacity:.65">('+g(a.driftGreen)+'/'+y(a.driftYellow)+'/'+r(a.driftRed)+')</span>':'')
