...
function buildCanonicalReviewHtml(o) {
  // Per-state assets: for every trigger that captured an interaction state
  // (crawl Phase A), render its pear canvas as a hidden backdrop, embed its
  // screenshot, and precompute highlight boxes for the newly-revealed text.
  // Parallel-indexed to triggers[side], so trigger i ↔ stateMeta[side][i].
  const buildStates = (trigList, side, baseClm) => {
    const blocks = [];
    const imgs = [];
    const meta = [];
    // A node's identity for "is this already in the base pear?" — CONTENT ONLY,
    // no position. The state CLM is captured on a separate crawl page load whose
    // coordinates drift from the base CLM, so a position-based key would mark
    // nearly every base node "new" and the delta would be the whole page. Text
    // by normalized value, media by src filename.
    const bkey = (n) =>
      n.kind === 'text'
        ? `t:${String(n.text || '').replace(/\s+/g, ' ').trim().toLowerCase()}`
        : `i:${String(n.src || '').split('/').pop().split('?')[0]}`;
    const baseSet = new Set(((baseClm && baseClm.nodes) || []).map(bkey));
    const baseDims = { sw: (baseClm && baseClm.width) || 0, sh: (baseClm && baseClm.height) || 0 };
    (trigList || []).forEach((t, idx) => {
      const st = t && t.state;
      if (!st || !st.clm) {
        meta.push(null);
        return;
      ...
      const clm = st.clm;
      const delta = (clm.nodes || []).filter((n) => !baseSet.has(bkey(n)));
      if (!delta.length) {
        meta.push(null); // click changed nothing genuinely new to show
        return;
      }
      // Opaque panel behind the delta so, in pear view, base content occluded by
      // a modal is hidden the way a real modal hides it (a screenshot does this
      // for free). For inline reveals (accordion/tab) the panel just backs the
      // new content. Then render ONLY the delta on top.
      const xs = delta.map((n) => n.x);
      const ys = delta.map((n) => n.y);
      const bx = Math.min(...xs) - 12;
      const by = Math.min(...ys) - 12;
      const bw = Math.max(...delta.map((n) => n.x + n.w)) - Math.min(...xs) + 24;
      const bh = Math.max(...delta.map((n) => n.y + n.h)) - Math.min(...ys) + 24;
      const pearId = `spd_${side}_${idx}`;
      const backing = `<div style="position:absolute;left:${bx}px;top:${by}px;width:${bw}px;height:${bh}px;background:#fff;border:1px solid #ddd;box-shadow:0 2px 10px rgba(0,0,0,.28)"></div>`;
      blocks.push(`<div class="pear statepear" id="${pearId}" style="display:none">${backing}${renderPearNodes({ width: clm.width, height: clm.height, nodes: delta })}</div>`);
      let shotId = null;
      if (st.shot) {
        shotId = `ss_${side}_${idx}`;
        imgs.push(`<img id="${shotId}" class="shot stateshot" alt="" draggable="false" src="${st.shot}" style="width:${clm.width}px;display:none">`);
      }
      const hi = delta.filter((n) => n.kind === 'text').map((n) => ({ x: n.x, y: n.y, w: n.w, h: n.h }));
      meta.push({ w: clm.width, h: clm.height, sw: baseDims.sw || clm.width, sh: baseDims.sh || clm.height, pearId, shotId, hi, n: delta.length, label: t.label || '' });
    });
    return { blocks, imgs, meta };
  };
  const St = { src: buildStates(o.triggers && o.triggers.source, 'src', o.sourceClm), tgt: buildStates(o.triggers && o.triggers.target, 'tgt', o.targetClm) };
  // Slim triggers for the client payload: keep geometry + reveal list, drop the
  // heavy per-state clm (already rendered into the hidden pear blocks above).
  const slimTrigs = (list) => (list || []).map((t) => ({ x: t.x, y: t.y, w: t.w, h: t.h, label: t.label, revealed: t.revealed || [] }));
  const data = {
    ...
    // Per-trigger captured interaction states (pear dims + highlight boxes).
    stateMeta: { source: St.src.meta, target: St.tgt.meta },
    audit: o.audit || null,
  };
...
<div id="bar">
  ...
  <span id="stateBar" style="display:none;gap:8px;align-items:center">
    <button id="bBack" style="background:var(--click);color:#fff;border:1px solid var(--click);padding:4px 10px;cursor:pointer">← Base</button>
    <small id="stateInfo" style="color:#cdb4ff"></small></span>
...
<div id="wrap">
  <div id="stage"><div id="inner">
    ...
    ${St.src.blocks.join('')}${St.tgt.blocks.join('')}${St.src.imgs.join('')}${St.tgt.imgs.join('')}
...
let panelOpen=false;
let stateView=null; // { side, idx } while a captured interaction state is shown
...
function render(){ if(stateView){ renderState(); return; } const ov=el('ov'); ov.innerHTML=''; const showAlign=el('tAlign').checked; const showMoved=el('tMoved').checked;
  ...
  if(el('tClick').checked && trigs){ const o=off(bgSide); const sm=(D.stateMeta&&D.stateMeta[bgSide])||[]; trigs.forEach((t,ti)=>{ const r=document.createElementNS(SVGNS,'rect');
    r.setAttribute('x',t.x+o.dx);r.setAttribute('y',t.y+o.dy);r.setAttribute('width',t.w);r.setAttribute('height',t.h);
    // Triggers that captured a state are filled more strongly — clicking them
    // opens the captured state; the rest just pop up their reveal list.
    const hasState=!!sm[ti];
    ...
    r.setAttribute('pointer-events','all'); r.style.cursor='pointer'; r.onclick=(e)=>{ e.stopPropagation(); if(hasState) enterState(bgSide,ti); else showReveal(t); }; ov.appendChild(r); }); }
...
// Interaction-state view: swap the backdrop to the captured state's pear (or its
// screenshot, honouring the Pear/Screenshot toggle), highlight the newly-revealed
// text in green, and show the "← Base" control. The base overlay is suppressed
// (render() defers to renderState) until you go back.
function enterState(side,idx){
  const m=D.stateMeta&&D.stateMeta[side]&&D.stateMeta[side][idx]; if(!m) return;
  stateView={side,idx}; hidePop();
  ...
  document.querySelectorAll('.statepear,.stateshot').forEach(e=>{ e.style.display='none'; });
  const useShot=backdrop==='shot' && m.shotId && el(m.shotId);
  let w,h;
  if(useShot){
    // Screenshot view: the full captured state screenshot (modal already occludes
    // base) — highlight only the new boxes on top.
    el(m.shotId).style.display='block'; w=m.w; h=m.h;
  } else {
    // Pear view: keep the BASE pear (at origin, no sync) and overlay only the
    // delta on its opaque panel, so it occludes the base beneath it like the
    // real modal — no confusing double-rendered base.
    const bp=el(side==='source'?'pearSrc':'pearTgt'); bp.style.transform='none'; bp.style.display='block';
    el(m.pearId).style.display='block'; w=m.sw; h=m.sh;
  }
  ...
  el('stateBar').style.display='inline-flex';
  el('stateInfo').textContent='State via “'+String(m.label||'trigger').slice(0,48)+'” · '+m.n+' new';
  renderState(); // stay at the current scroll height (do not jump to top)
}
function renderState(){ const ov=el('ov'); ov.innerHTML=''; if(!stateView) return;
  const m=D.stateMeta[stateView.side][stateView.idx]; if(!m) return;
  for(const b of (m.hi||[])){ const r=document.createElementNS(SVGNS,'rect');
    r.setAttribute('x',b.x);r.setAttribute('y',b.y);r.setAttribute('width',b.w);r.setAttribute('height',b.h);
    r.setAttribute('fill','#22c55e');r.setAttribute('fill-opacity','0.18');r.setAttribute('stroke','#22c55e');r.setAttribute('stroke-width','2'); ov.appendChild(r); } }
function exitState(){ if(!stateView) return; stateView=null; el('stateBar').style.display='none';
  document.querySelectorAll('.statepear,.stateshot').forEach(e=>{ e.style.display='none'; }); applyView(); }
...
el('bSide').onclick=()=>{ exitState(); bgSide=bgSide==='source'?'target':'source'; applyView(); };
if(el('bPear')){
  el('bPear').onclick=()=>{ backdrop='pear'; el('bPear').classList.add('on'); el('bShot').classList.remove('on'); if(stateView) enterState(stateView.side,stateView.idx); else applyView(); };
  el('bShot').onclick=()=>{ backdrop='shot'; el('bShot').classList.add('on'); el('bPear').classList.remove('on'); if(stateView) enterState(stateView.side,stateView.idx); else applyView(); };
}
el('bBack').onclick=()=>exitState();
