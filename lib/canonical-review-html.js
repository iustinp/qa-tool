...
function fillSide(){ const s=el('side'); const txt=i=>!i.img;
  ...
  el('stat').innerHTML='<small>'
    +(a.healthScore!=null?'<b title="overall page health — HIGHER is BETTER, 100 = perfect (0.7·content + 0.3·(100−drift)). Text only; images not yet scored.">health '+a.healthScore+'/100</b>':'')
    +(a.contentScore!=null?' · <span title="content completeness — HIGHER is BETTER, 100% = all source content present (missing-dominant)">content '+a.contentScore+'%</span> <span style="opacity:.65">('+(a.matchedCount!=null?a.matchedCount:'?')+'✓ '+r(a.missingCount)+' miss '+y(a.extraCount)+' extra)</span>':'')
    +(a.driftScore!=null?' · <span title="positional drift of matched elements — LOWER is better, 0 = none (red-dominant)">drift '+a.driftScore+'</span> <span style="opacity:.65">('+g(a.driftGreen)+'/'+y(a.driftYellow)+'/'+r(a.driftRed)+')</span>':'')
