...
function buildCanonicalReviewHtml(o) {
  ...
  const buildStates = (trigList, side) => {
    const blocks = [];
    const imgs = [];
    const meta = [];
    (trigList || []).forEach((t, idx) => {
      ...
      const clm = st.clm;
      const pearId = `sp_${side}_${idx}`;
      blocks.push(`<div class="pear statepear" id="${pearId}" style="display:none">${renderPearNodes(clm)}</div>`);
      let shotId = null;
      if (st.shot) {
      ...
      const revSet = new Set((st.revealed || []).map(clean));
      const hi = (clm.nodes || [])
        .filter((n) => n.kind === 'text' && revSet.has(clean(n.text)))
        .map((n) => ({ x: n.x, y: n.y, w: n.w, h: n.h }));
      meta.push({ w: clm.width, h: clm.height, pearId, shotId, hi, n: (st.revealed || []).length, label: t.label || '' });
    });
    return { blocks, imgs, meta };
  };
  const St = { src: buildStates(o.triggers && o.triggers.source, 'src'), tgt: buildStates(o.triggers && o.triggers.target, 'tgt') };
...
function enterState(side,idx){
  ...
  el(useShot?m.shotId:m.pearId).style.display='block';
  el('inner').style.width=m.w+'px'; el('inner').style.height=m.h+'px';
  const ov=el('ov'); ov.setAttribute('width',m.w); ov.setAttribute('height',m.h); ov.setAttribute('viewBox','0 0 '+m.w+' '+m.h);
  el('stateBar').style.display='inline-flex';
  el('stateInfo').textContent='State via “'+String(m.label||'trigger').slice(0,48)+'” · '+m.n+' revealed';
  el('stage').scrollTo({top:0,behavior:'auto'});
  renderState();
}
