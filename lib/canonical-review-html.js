...
function showState(){
  ...
  // #62 — the state backdrop is now a VIEWPORT shot (~one screen tall) scrolled to
  // the trigger, with the reveal near its top. Reset the stage to the top of that
  // frame so the reveal is visible (a preserved deep base-scroll would land past
  // the short backdrop and show blank canvas).
...
function exitToBase(){ navStack=[]; el('stateBar').style.display='none'; hidePop();
  ...
  const stg=el('stage'); if(stg){ stg.scrollTop=baseScroll.top; stg.scrollLeft=baseScroll.left; } }
