...
function buildCanonicalReviewHtml(o) {
  ...
  const buildStates = (trigList, side, baseClm) => {
    ...
    const imgs = [];
    const meta = [];
    // A node's identity for "is this already in the base pear?" — kind + text/src
    // + coarse position (so a modal's re-rendered base content is recognized and
    // only the genuinely-new nodes form the delta).
    const bkey = (n) => `${n.kind}:${(n.text || n.src || '').slice(0, 40)}:${Math.round(n.x / 6)}:${Math.round(n.y / 6)}`;
