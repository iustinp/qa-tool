...
function slimItem(it) {
  ...
  if (it.bid != null) o.b = it.bid; // inline-paragraph id (drift scored per paragraph)
  if (it.counterpart) {
...
/**
 * #63 Cross-side slide pairing + registered drift for the hidden layer.
 * Pairs source↔target slides by text-set overlap (order/index/position independent,
 * so autoplay + reorder are handled), matches their text nodes, and annotates each
 * node IN PLACE with the normal item shape so the review draws it through the same
 * box() path: `i`, `m` (aligned → solid box, else dotted), and for aligned nodes
 * `cx,cy` = the counterpart node REGISTERED onto this side (the counterpart shifted
 * by the matched slides' top-left delta). The registration removes the carousel
 * offset, so the node→counterpart connector shows the TRUE intra-slide drift rather
 * than the huge horizontal gap of two slides sitting at different carousel indices.
 */
function pairHiddenSlides(srcItems, srcNodes, tgtItems, tgtNodes) {
  const norm = (s) => (s || '').replace(/\s+/g, ' ').trim().toLowerCase();
  let gi = 0;
  const prep = (items, nodes) => {
    ...
    for (const n of nodes) {
      ...
      n.cx = null;
      n.cy = null;
      if (!byItem.has(n.itemIdx)) byItem.set(n.itemIdx, []);
      byItem.get(n.itemIdx).push(n);
    }
    return (items || []).map((it) => {
      const ns = byItem.get(it.idx) || [];
      return { it, nodes: ns, set: new Set(ns.map((n) => norm(n.text)).filter(Boolean)) };
    });
  };
  const S = prep(srcItems || [], srcNodes || []);
  const T = prep(tgtItems || [], tgtNodes || []);
  const overlap = (a, b) => {
    if (!a.set.size || !b.set.size) return 0;
    let c = 0;
    for (const x of a.set) if (b.set.has(x)) c += 1;
    return c / Math.min(a.set.size, b.set.size);
  };
  // Greedy best-overlap: score every source×target slide pair, assign highest first.
  const cands = [];
  S.forEach((s, si) => T.forEach((t, ti) => { const ov = overlap(s, t); if (ov >= 0.5) cands.push({ si, ti, ov }); }));
  ...
  for (const c of cands) {
    ...
    const s = S[c.si];
    const t = T[c.ti];
    // Match the two slides' text runs by content.
    const tByText = new Map();
    for (const tn of t.nodes) { const k = norm(tn.text); if (k && !tByText.has(k)) tByText.set(k, tn); }
    const matched = [];
    const tUsed = new Set();
    for (const sn of s.nodes) {
      const tn = tByText.get(norm(sn.text));
      if (!tn || tUsed.has(tn)) continue;
      tUsed.add(tn);
      matched.push([sn, tn]);
    }
    if (!matched.length) continue;
    // Register by the MEDIAN matched-run delta rather than the slide-box top-left:
    // a production slide's box often doesn't tightly bound its content (absolute-
    // positioned slide content), so box-top-left anchoring injects a large constant
    // offset that swamps the real drift. The median run delta cancels that systematic
    // inset — same principle as the main-pear drift (median neighbour shift) — so the
    // residual per run is its TRUE local drift.
    const mdx = median(matched.map(([sn, tn]) => sn.x - tn.x));
    const mdy = median(matched.map(([sn, tn]) => sn.y - tn.y));
    for (const [sn, tn] of matched) {
      sn.m = true; sn.cx = tn.x + mdx; sn.cy = tn.y + mdy;
      tn.m = true; tn.cx = sn.x - mdx; tn.cy = sn.y - mdy;
    }
  }
}

function buildCanonicalReviewHtml(o) {
  // #63: annotate the hidden nodes (in place) with cross-side match status +
  // registered-counterpart coords, before they're embedded and rendered.
  pairHiddenSlides(o.sourceClm?.hiddenItems, o.sourceClm?.hiddenNodes, o.targetClm?.hiddenItems, o.targetClm?.hiddenNodes);
  ...
  const data = {
    ...
    // Capture DPR per side, so the client can size each screenshot to its TRUE
    // document CSS dimensions (natural px / dpr) rather than assuming it equals the
    // extraction-time width — which breaks when the page's document width differs
    // from `*Dims.w` (horizontal overflow appearing before the full-page shot).
    sourceDpr: o.sourceClm?.dpr || 1,
    targetDpr: o.targetClm?.dpr || 1,
  ...
  const hiddenGhostImgs = (items) =>
    (items || [])
      ...
      .join('\n');
  // The captured slide set includes each slider's currently-VISIBLE slide (so the
  // set is deterministic + symmetric across sides). But that slide's text is already
  // painted crisply by the NORMAL pear at the same spot, so also drawing it in the
  // hidden layer produces two slightly-misaligned copies (eye strain). Keep the
  // visible slide's faded ghost IMAGE (so the row reads as continuous) but drop its
  // hidden pear TEXT — the normal pear carries it.
  const hiddenPearNodes = (clm) => renderPearNodes({ nodes: (clm?.hiddenNodes || []).filter((n) => !n.visibleNow) }, { ghost: true });
  const srcHiddenCanvas = hiddenGhostImgs(o.sourceClm?.hiddenItems) + '\n' + hiddenPearNodes(o.sourceClm);
  const tgtHiddenCanvas = hiddenGhostImgs(o.targetClm?.hiddenItems) + '\n' + hiddenPearNodes(o.targetClm);
...
${fontHead(fonts)}
<style>
  :root{--src:#00a000;--tgt:#e00000;--align:#0060ff;--moved:#ff8800;--click:#9b30ff;--hidden:#00b3b3;--reflow:#94a3b8}
  *{box-sizing:border-box}
  ...
  #bar{position:fixed;top:0;left:0;right:0;z-index:10;background:#111;border-bottom:1px solid #444;
    padding:6px 10px;display:block;gap:14px;align-items:center;flex-wrap:wrap}
  #bar b{color:#fff}
  ...
  /* Pair highlight: hovering a box or its connector lights up the whole pair
     (source box + target box + connector) and dims every other connector, so a
     single relationship stands out even where dozens crowd together. */
  #ov.hovering path.conn:not(.hl){opacity:0.1}
  #ov.hovering g.box:not(.hl) rect{opacity:0.28}
  #ov path.conn.hl{stroke-width:3;opacity:1;filter:drop-shadow(0 0 4px #fff)}
  #ov g.box.hl rect{stroke-width:4;filter:drop-shadow(0 0 5px #fff)}
...
<div id="bar">
  ...
  <span class="seg"><button id="bSide" class="on" title="Which page fills the stage. Click — or press Spacebar — to toggle between Source and Target.">source / TARGET</button></span>
  ${hasShots ? '<span class="seg"><button id="bPear" title="Backdrop: the reconstructed text canvas (the &quot;pear&quot;) — shows exactly where the tool reads each text run.">Pear</button><button id="bShot" class="on" title="Backdrop: the actual captured page screenshot.">Screenshot</button></span>' : ''}
  <label title="Draw the bounding box of every SOURCE text run over the backdrop (green, or blue when Matched is on)."><span class="sw" style="background:var(--src)"></span><input type="checkbox" id="tSrc" checked>Source boxes</label>
  <label title="Draw the bounding box of every TARGET text run over the backdrop (red, or blue when Matched is on)."><span class="sw" style="background:var(--tgt)"></span><input type="checkbox" id="tTgt" checked>Target boxes</label>
  <label title="Colour matched pairs blue (they align across source/target) — vs green source / red target"><span class="sw" style="background:var(--align)"></span><input type="checkbox" id="tAlign" checked>Matched</label>
  <label title="Label each box with its reading-order index number."><input type="checkbox" id="tIdx">Indices</label>
  <label title="Draw a line from each source text run to its matched target counterpart, coloured by positional drift (green &le;20px, yellow &le;40px, red &gt;40px)."><input type="checkbox" id="tLine" checked>Connectors</label>
  <label title="Outline clickable regions (dashed) and, on top, the interactive state-entry boxes you can click to descend into revealed content."><span class="sw" style="background:var(--click)"></span><input type="checkbox" id="tClick">Clickable</label>
  ...
  <label title="Off-screen slider/carousel slides laid out at their natural flow position. Slides are paired across sides by content: each run is boxed solid when aligned (green source / red target, blue when Aligned is on) or dotted when only on this side. Connectors show the intra-slide drift with the carousel offset removed."><span class="sw" style="background:var(--hidden)"></span><input type="checkbox" id="tHidden">Hidden content</label>
  <span id="connLegend" style="display:none"><small>drift
    ...
    <span class="sw" style="background:#ef4444"></span>&gt;40
    <span class="sw" style="background:var(--reflow)"></span>re-split/image (not scored)</small></span>
  <button id="bReset" title="Undo a pinned alignment: clicking a box rigidly shifts the other side so that pair lines up. This clears that shift." style="display:none;background:#333;color:#ddd;border:1px solid #555;padding:4px 10px;cursor:pointer">Reset align</button>
  <span class="seg"><button id="bPanel" title="Open the side panel listing every missing / extra / matched text run.">Diffs</button></span>
...
<script>
const D=${json};
// Reflow groups: the two sides split the same block into different run COUNTS, so a
// run's raw counterpart is the OTHER side's differently-segmented box — e.g. a source
// role run ("National Manager…") counterparts the START of the target's merged
// "Name, Role" box, which reads as a big sideways drift though nothing moved. Re-base
// every reflow run's counterpart to the run translated by the GROUP's uniform shift
// (its first-element delta, the same anchor the click-sync uses). One pass here keeps
// the connector line, its drift COLOUR (computeDrifts) and sync all consistent; cleanly
// matched runs (no .g) are untouched. Display-only — scores are computed server-side.
for(const it of (D.source||[])){ if(it.g){ it.cx=it.x+(it.g.tx-it.g.sx); it.cy=it.y+(it.g.ty-it.g.sy); } }
for(const it of (D.target||[])){ if(it.g){ it.cx=it.x+(it.g.sx-it.g.tx); it.cy=it.y+(it.g.sy-it.g.ty); } }
const SVGNS='http://www.w3.org/2000/svg';
let bgSide='target', backdrop='${hasShots ? 'shot' : 'pear'}';
...
let hoverKey=null; // pair key (pN) currently hovered — drives the side-switch scroll-sync
let stickyKey=null; // after a side-flip, the highlight is PINNED to this pair (so live
  // hover doesn't steal it onto whatever box now sits at the anchored spot); released the
  // first time the cursor crosses empty space, after which free hover resumes.
let panelOpen=false;
...
let shotCss={source:null,target:null}; // each screenshot's TRUE document CSS size (natural px / dpr), measured on load
// Size a screenshot to its real document dimensions so it registers 1:1 with the
// pear coordinate space, regardless of whether the shot's document width matches the
// extraction-time width (they differ when horizontal overflow appears before the
// full-page shot — the shot would otherwise be forced to *Dims.w and shrunk).
function sizeShot(side){ const img=el(side==='source'?'shotSrc':'shotTgt'); if(!img||!img.naturalWidth) return;
  const dpr=(side==='source'?D.sourceDpr:D.targetDpr)||1;
  const w=Math.round(img.naturalWidth/dpr), h=Math.round(img.naturalHeight/dpr);
  img.style.width=w+'px'; img.style.height=h+'px'; shotCss[side]={w,h}; }
...
function applyView(){
  ...
  // BOTH sides share ONE canvas — the larger page's dimensions. A box at a given
  // document position then sits at the same scroll offset regardless of side, which
  // is what makes the side-switch scroll-sync land the counterpart under the cursor,
  // and stops the smaller page from clamping the shared scroll extent.
  ...
  const g=D.graph&&D.graph[bgSide];
  let cw=Math.max(D.sourceDims.w, D.targetDims.w);
  let ch=Math.max(D.sourceDims.h, D.targetDims.h);
  // Each screenshot is displayed at its TRUE document size (natural px / dpr, set in
  // sizeShot on load), which can exceed the extraction-time *Dims when the page's
  // document width differs from it (horizontal overflow) — grow the canvas to fit
  // EITHER side's shot so no shot is shrunk and content registers 1:1 with the pears.
  for(const s of ['source','target']){ const sc=shotCss[s]; if(sc){ cw=Math.max(cw,sc.w); ch=Math.max(ch,sc.h); } }
...
function box(it,color,side){ const g=document.createElementNS(SVGNS,'g');
  g.setAttribute('class','box');g.setAttribute('data-k',pairKey(it));
...
// Global connector bow: control point offset perpendicular to the chord by this
// fraction of the connector length. One consistent sign so all arcs bow the same
// way — collinear straight lines (many pairs share an x or a y) would stack on top
// of each other, but as arcs of differing length/position they fan apart.
const CONN_CURVE=0.14;
// Highlight key shared by a matched pair's source box, target box and connector.
// Matched source is "N", its target counterpart "N:0" -> both key "pN". Unmatched
// boxes key to themselves so hovering one just lights itself (no false pairing).
function pairKey(it){ const s=String(it.i); return it.m ? 'p'+s.split(':')[0] : 'u'+s; }
function median(a){ if(!a.length) return 0; const s=[...a].sort((x,y)=>x-y); const m=s.length>>1; return s.length%2?s[m]:(s[m-1]+s[m])/2; }
// Drift over cleanly-MATCHED text runs (reflow re-split groups and images are excluded —
// their per-run position isn't a drift signal). EXACT mirror of driftBuckets in
// text-layout-compare.js. The wrap problem only exists across LINES, so we only group there:
//   UNFUSED paragraph — all runs on ~one line (a row of separate items, e.g. footer links):
//     grade EACH run on its own; no hidden grouping.
//   FUSED paragraph — runs wrap across MULTIPLE lines (one flowing paragraph): grade ONCE at
//     the first run; siblings sit where the text re-wrapped (reflow, not drift) so they're
//     left UNSCORED and drawn gray — you still see where a piece jumped, it just isn't counted.
// Only scored runs get a colour, and the stat counts scored runs, so coloured lines == stat.
function computeDrifts(){
  const M=D.source.filter(it=>it.m&&it.cx!=null&&!it.g&&!it.img);
  const byBid={};
  M.forEach((it,i)=>{ it._bk=(it.b!=null?'b'+it.b:'s'+i); it._scored=false; it._drift=0; (byBid[it._bk]=byBid[it._bk]||[]).push(it); });
  const driftOf=(it)=>{ const near=M.filter(o=>o._bk!==it._bk).sort((a,b)=>Math.abs(a.y-it.y)-Math.abs(b.y-it.y)).slice(0,DRIFT_K);
    if(!near.length) return 0;
    const mdx=median(near.map(o=>o.cx-o.x)), mdy=median(near.map(o=>o.cy-o.y));
    return Math.max(Math.abs((it.cx-it.x)-mdx),Math.abs((it.cy-it.y)-mdy)); };
  for(const k in byBid){ const runs=byBid[k];
    const ys=runs.map(r=>r.y); const h=median(runs.map(r=>r.h))||16;
    const multiLine=Math.max(...ys)-Math.min(...ys)>h*0.5;
    if(multiLine){ const a=runs.reduce((best,it)=>(!best||it.y<best.y||(it.y===best.y&&it.x<best.x))?it:best,null); a._drift=driftOf(a); a._scored=true; }
    else { for(const it of runs){ it._drift=driftOf(it); it._scored=true; } }
  } }
function connColor(it){
  // Only SCORED runs carry a colour: every run of an unfused single-line row, plus the first
  // run of a fused multi-line paragraph. Fused siblings, reflow re-split pairs and images are
  // present-but-not-scored — drawn neutral so they don't read as drift (mirrors the stat).
  if(!it._scored) return 'var(--reflow)';
  const d=it._drift||0;
...
function connector(it){ const so=off('source'), to=off('target');
  const x1=it.x+so.dx, y1=it.y+so.dy, x2=it.cx+to.dx, y2=it.cy+to.dy;
  const dx=x2-x1, dy=y2-y1;
  // Quadratic bezier bowed perpendicular to the chord (see CONN_CURVE): the offset
  // CONN_CURVE*(-dy,dx) is a vector normal to (dx,dy) whose magnitude scales with length.
  const qx=(x1+x2)/2 + CONN_CURVE*dy, qy=(y1+y2)/2 - CONN_CURVE*dx;
  ...
  p.setAttribute('class','conn');p.setAttribute('data-k',pairKey(it)); return p; }
function render(){ if(navStack.length){ renderStateOverlay(); return; } const ov=el('ov'); ov.innerHTML=''; const showAlign=el('tAlign').checked;
  ...
  // Connectors (under the boxes): one per index that has a counterpart. (A relocated
  // element — formerly "moved" — is now just a matched item with a large drift, so it
  // gets a connector + drift colour like any other; no special-casing.)
  if(lines) for(const it of D.source){ if(it.cx==null) continue; ov.appendChild(connector(it)); }
  ...
  // "aligned" (blue) even when the boxes don't overlap. Green = missing on target,
  // red = extra on target.
  // Largest-area first → smaller boxes paint LAST (on top). The active side's boxes
  // capture over their whole area (pointer-events:all, for click-to-sync), so without
  // this a big box drawn over a small one would swallow the small box's hover/clicks.
  const byArea=arr=>[...arr].sort((a,b)=>(b.w*b.h)-(a.w*a.h));
  if(el('tSrc').checked) for(const it of byArea(D.source)){
    ov.appendChild(box(it,(showAlign&&it.m)?'var(--align)':'var(--src)','source')); }
  if(el('tTgt').checked) for(const it of byArea(D.target)){
  ...
  // natural-flow positions. Slides are PAIRED across sides by content (pairHiddenSlides,
  // server-side), so each text run carries the normal item shape: m = aligned (has a
  // counterpart on the other side), else only-source / only-target. We draw each run
  // through the SAME box() path as the main pears, so the styling matches the whole
  // tool: unmatched -> DOTTED side colour (only-source green / only-target red), aligned
  // -> SOLID side colour, and aligned -> BLUE when "Aligned" is checked. The visible
  // slide is skipped (its box lives in the normal pear layer). Connectors (when on)
  // run each run -> its REGISTERED counterpart, so the line length/colour is the true
  // intra-slide drift (carousel offset removed by the registration) - a wrong slide
  // position or a missing slide shows up as a large drift / an unmatched dotted box.
  if(el('tHidden').checked && D.hidden && D.hidden[bgSide]){ const o=off(bgSide); const H=D.hidden[bgSide];
    const sideCol=bgSide==='source'?'var(--src)':'var(--tgt)';
    // Item extents (faded teal) — the text/background comes from the #hiddenSrc/Tgt
    // pear layer beneath; here we only draw the item frame.
    ...
    // Registered-drift connectors (under the boxes) for matched runs.
    if(lines) for(const n of (H.nodes||[])){ if(n.visibleNow||!n.m||n.cx==null) continue;
      const p=document.createElementNS(SVGNS,'line');
      p.setAttribute('x1',n.x+o.dx);p.setAttribute('y1',n.y+o.dy);p.setAttribute('x2',n.cx+o.dx);p.setAttribute('y2',n.cy+o.dy);
      const d=Math.max(Math.abs(n.x-n.cx),Math.abs(n.y-n.cy));
      p.setAttribute('stroke',d<=CONN_GOOD?'#22c55e':d<=CONN_OKISH?'#eab308':'#ef4444');p.setAttribute('stroke-width','1.5');p.setAttribute('opacity','0.7'); ov.appendChild(p); }
    // Text-run boxes coloured by match status (dotted for unmatched via box()).
    for(const n of (H.nodes||[])){ if(n.visibleNow) continue;
      const col=(showAlign&&n.m)?'var(--align)':sideCol;
      ov.appendChild(box({x:n.x,y:n.y,w:Math.max(n.w,8),h:n.h,m:n.m,i:n.i}, col, bgSide)); }
  }
...
function fillSide(){ const s=el('side'); const txt=i=>!i.img;
  const miss=D.source.filter(i=>!i.m&&txt(i)); const extra=D.target.filter(i=>!i.m&&txt(i));
  const missImg=D.source.filter(i=>!i.m&&i.img); const extraImg=D.target.filter(i=>!i.m&&i.img);
  ...
  // Health = 0-100, higher = worse. Global decomposes into content (missing-dominant)
  // and drift (red-dominant); each shows its breakdown. See lib/text-layout-compare.js.
  const a=D.audit||{};
  const g=(v)=>'<span style="color:#22c55e">'+(v||0)+'</span>', y=(v)=>'<span style="color:#eab308">'+(v||0)+'</span>', r=(v)=>'<span style="color:#ef4444">'+(v||0)+'</span>';
  el('stat').innerHTML='<small>'
    +(a.healthScore!=null?'<b title="overall page health — HIGHER is BETTER, 100 = perfect (0.7·content + 0.3·(100−drift)). Text only; images not yet scored.">General Health '+a.healthScore+'%</b>':'')
    +(a.contentScore!=null?' · <span title="content completeness — HIGHER is BETTER, 100% = all source content present (missing-dominant)">Content Completeness '+a.contentScore+'%</span> <span style="opacity:.65">('+(a.matchedCount!=null?a.matchedCount:'?')+'✓ '+r(a.missingCount)+' miss '+y(a.extraCount)+' extra)</span>':'')
    +(a.driftScore!=null?' · <span title="positional drift, graded per paragraph but counted per matched text run — one tally per coloured connector on the page (re-split blocks and images are neutral / not counted) — LOWER is better, 0 = none (red-dominant)">Drifting texts '+a.driftScore+'%</span> <span style="opacity:.65">('+g(a.driftGreen)+'/'+y(a.driftYellow)+'/'+r(a.driftRed)+')</span>':'')
    +(a.imageMatchedCount!=null?' · <span title="images shown for reference — NOT folded into the score (matching not yet validated)">img '+a.imageMatchedCount+'✓/'+a.imageMissingCount+'✗/'+a.imageExtraCount+'+</span>':'')+'</small>'; }
function escapeHtml(x){return String(x).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]));}
function flipSide(){
  // Switching sides must leave the state view (states are per-side). Keep the viewport
  // ANCHORED where it is, so you stay on the box you started from and watch where its
  // counterpart sits on the other side — the connector then shows exactly how far the
  // text jumped. (Both sides share one canvas size, so "the same scroll position"
  // means the same document region on either side; the counterpart is drawn at its
  // true absolute coords, so its offset reads correctly against the anchored box.)
  const stg=el('stage');
  const base=!navStack.length && stg;
  ...
  const docX=base?(stg.scrollLeft-hiddenLeftPad):null;
  const keep=base?{top:stg.scrollTop,left:stg.scrollLeft}:baseScroll;
  bgSide=bgSide==='source'?'target':'source';
  exitToBase(keep);
  if(base && docX!=null) stg.scrollLeft = docX + hiddenLeftPad;
  // Re-assert the hovered pair's highlight (render() rebuilt the SVG) and PIN it, so the
  // connector and both boxes stay lit across the swap even as the mouse moves — otherwise
  // the box now sitting at the anchored spot (a different pair) would steal the hover.
  // The pin releases the first time the cursor crosses empty space (see the mouseout).
  if(hoverKey){ stickyKey=hoverKey; setHl(hoverKey); }
}
el('bSide').onclick=flipSide;
if(el('bPear')){
...
['tSrc','tTgt','tAlign','tIdx','tLine','tClick','tReveal'].forEach(id=>el(id).onchange=render);
...
document.addEventListener('keydown',(e)=>{ if(e.key==='Escape'){ hidePop(); return; }
  // Spacebar toggles source/target (with the hover scroll-sync). Ignore it while a
  // form control is focused, and stop the page from scrolling on the space press.
  if(e.key===' '||e.code==='Space'){ if(/^(INPUT|TEXTAREA|SELECT|BUTTON)$/.test(e.target.tagName||'')) return; e.preventDefault(); flipSide(); } });
// A mouse-clicked toolbar control keeps keyboard focus, so the next spacebar would
// toggle IT instead of flipping sides. Drop that focus back to the document after a
// click, so spacebar always flips without having to click empty space first. (Native
// keyboard use is untouched — Tabbing to a control still focuses it, and space still
// toggles it while it's the focused element; this only clears focus left by a click.)
el('bar').addEventListener('click',()=>{ const a=document.activeElement; if(a&&a.blur&&el('bar').contains(a)) a.blur(); });
// Pair highlight on hover: light up a box/connector's whole pair, dim the rest.
function keyOf(n){ while(n&&n!==el('ov')){ if(n.getAttribute&&n.getAttribute('data-k')) return n.getAttribute('data-k'); n=n.parentNode; } return null; }
function setHl(k){ const ov=el('ov'); ov.querySelectorAll('.hl').forEach(e=>e.classList.remove('hl'));
  if(k){ ov.classList.add('hovering'); ov.querySelectorAll('[data-k="'+k+'"]').forEach(e=>e.classList.add('hl')); }
  else ov.classList.remove('hovering'); }
el('ov').addEventListener('mouseover',e=>{
  // While pinned (just flipped sides), ignore box hovers so the studied pair stays lit.
  if(stickyKey) return;
  const k=keyOf(e.target); if(k){ hoverKey=k; setHl(k); } });
el('ov').addEventListener('mouseout',e=>{ if(!keyOf(e.relatedTarget)){
  // Crossing into empty space releases the pin and returns to normal hover.
  stickyKey=null; hoverKey=null; setHl(null); } });
computeDrifts(); fillSide();
el('side').style.display=panelOpen?'block':'none'; el('bPanel').classList.toggle('on',panelOpen);
// Measure each screenshot's true size once it decodes (or immediately if cached),
// then re-lay-out so the canvas fits it and it's not shrunk to the pear width.
if(el('shotSrc')){ for(const side of ['source','target']){ const img=el(side==='source'?'shotSrc':'shotTgt'); if(!img) continue;
  if(img.complete && img.naturalWidth) sizeShot(side); else img.addEventListener('load',()=>{ sizeShot(side); applyView(); }); } }
applyView();
</script></body></html>`;
}
