...
<div id="bar">
  ...
  <span class="seg"><button id="bPanel" class="on">Diffs</button></span>
...
let highlight=null, hlTimer=null; // { side, i } — a diff-row's box, glowing
let panelOpen=true;
...
function render(){ const ov=el('ov'); ov.innerHTML=''; const showAlign=el('tAlign').checked; const showMoved=el('tMoved').checked;
  ...
  // Temporary glow on a diff-row's box (drawn last, always, regardless of layer toggles).
  if(highlight){ const arr=highlight.side==='source'?D.source:D.target; const it=arr.find(x=>String(x.i)===String(highlight.i));
    if(it){ const o=off(highlight.side); const gr=document.createElementNS(SVGNS,'rect');
      gr.setAttribute('x',it.x+o.dx-4);gr.setAttribute('y',it.y+o.dy-4);gr.setAttribute('width',(it.w||1)+8);gr.setAttribute('height',(it.h||1)+8);
...
// Clicking a diff row jumps to its box: switch to the side that actually shows
// that text (missing/moved → source, extra → target), scroll it into view
// (accounting for any sync offset), and glow its rectangle briefly.
function onRowClick(side,i,y){
  highlight={side,i};
  if(bgSide!==side){ bgSide=side; applyView(); } else render();
  el('stage').scrollTo({top:Math.max(0,(y+off(side).dy)-140),behavior:'smooth'});
...
function fillSide(){ const s=el('side'); const miss=D.source.filter(i=>!i.m&&!i.mv); const extra=D.target.filter(i=>!i.m&&!i.mv); const moved=D.source.filter(i=>i.mv);
  const rows=(arr,cls,side,suffix)=>arr.map(i=>'<div class="row '+cls+'" data-side="'+side+'" data-i="'+i.i+'" data-y="'+i.y+'">'+i.i+'. '+escapeHtml(i.t)+(suffix?suffix(i):'')+'</div>').join('');
  s.innerHTML='<h4>Missing on target ('+miss.length+')</h4>'+rows(miss,'miss','source')
    +'<h4>Moved ('+moved.length+')</h4>'+rows(moved,'moved','source',i=>' <small>y'+i.y+'→'+i.cy+'</small>')
    +'<h4>Extra on target ('+extra.length+')</h4>'+rows(extra,'extra','target');
  s.querySelectorAll('.row').forEach(r=>r.onclick=()=>onRowClick(r.dataset.side,r.dataset.i,+r.dataset.y));
  const a=D.audit||{};
