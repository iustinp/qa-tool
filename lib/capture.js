  ...
  for (let attempt = 0; attempt <= ANTI_BOT_CONFIG.maxRetries; attempt++) {
    ...
    try {
      ...
      if (options.collectCanonicalLayout) {
        ...
        // extractor runs in page.evaluate and can't screenshot. Per item, crawl-style:
        // neutralise overflow+transform on its clipping/transform ancestors so it
        // lays out at its natural spot, image just that element, RESTORE immediately
        // (localised so the base full-page screenshot below sees a pristine page).
        ...
        if (hItems.length) {
          ...
          for (const it of hItems.slice(0, 60)) {
            let h = null;
            try { h = await page.$(`[data-ppd-hidden="${it.idx}"]`); } catch { /* ignore */ }
            if (!h) continue;
            try {
              await h.evaluate((el) => {
                window.__ppdHG = [];
              ...
              const buf = await h.screenshot({ timeout: 2500 });
              it.ghost = `data:image/png;base64,${buf.toString('base64')}`;
              ghosted += 1;
          ...
          try { await page.evaluate(() => { document.querySelectorAll('[data-ppd-hidden]').forEach((e) => e.removeAttribute('data-ppd-hidden')); window.scrollTo(0, 0); }); } catch { /* ignore */ }
