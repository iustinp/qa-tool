...
async function stabilizePageForCapture(page, opts = {}) {
  ...
  // Recipe ignore list for THIS side (source or target): each matched element and
  // its whole subtree is removed before any extraction, so the ignored region is
  // gone from screenshot, text, layout and OCR at once. `ignoreMatched` reports
  // per-selector counts so a selector that matched nothing can be warned about.
  ...
  return page.evaluate(
    ({ freeze, selectors, ocrContrast, ignoreSelectors }) => {
      ...
      // Recipe ignores (removed from this side only).
      const ignoreMatched = [];
