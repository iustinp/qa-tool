...
async function stabilizePageForCapture(page, opts = {}) {
  ...
  const removeModalSelectors = opts.removeModalSelectors || [
    '.modal.block', // EDS modal block
    '[aria-modal="true"]',
    '.modal-backdrop',
    '.overlay-backdrop',
  ];
  return page.evaluate(
    ({ freeze, selectors }) => {
      ...
      return { modalsRemoved, froze, intervalsCleared, mediaPaused, scrollY };
    },
    { freeze: freezeAnimations, selectors: removeModalSelectors }
  );
}
