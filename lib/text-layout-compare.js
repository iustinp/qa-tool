...
function compareTextLayout(source, target, opts = {}) {
  ...
  const flush = () => {
    ...
    if (reflowed) {
      ...
      for (const it of curSeg.src) missing.push({ text: it.text, x: it.x, y: it.y, key: it.key });
      for (const it of curSeg.tgt) extra.push({ text: it.text, x: it.x, y: it.y, key: it.key });
  ...
  // Moved/reordered reconciliation: the same text can appear on both pages but
  // out of sequence (reordered paragraphs, or a positioned/sticky bar whose
  // document coord doesn't match its visual spot), so the order-based LCS leaves
  // it as both missing and extra. Pair a leftover missing item with a leftover
  // extra item of equal (fuzzy) text → it's MOVED, not missing.
  ...
  for (const mi of missing) {
    ...
    if (found >= 0) {
      ...
      moved.push({
        ...
        sourceX: mi.x,
        sourceY: mi.y,
        targetX: extra[found].x,
        targetY: extra[found].y,
      });
    } else {
      stillMissing.push(mi);
    }
  }
  const stillExtra = extra.filter((_, i) => !usedExtra.has(i));
  missing.length = 0;
  missing.push(...stillMissing);
  extra.length = 0;
  extra.push(...stillExtra);
...
function alignForOverlay(source, target, opts = {}) {
  ...
  // Moved reconciliation (same as the audit): pair a leftover unmatched source
  // item with a leftover unmatched target item of equal (fuzzy) text elsewhere,
  // and flag both as moved (with counterpart), so reordered/repositioned content
  // reads as "moved" in the overlay instead of missing + extra.
  const eq = makeKeyEq(opts);
  const freeTgt = targetItems.filter((t) => !t.matched);
  const usedT = new Set();
  for (const s of sourceItems) {
    if (s.matched) continue;
    const t = freeTgt.find((c) => !usedT.has(c) && eq(s.key, c.key));
    if (t) {
      usedT.add(t);
      s.moved = true;
      s.counterpart = t;
