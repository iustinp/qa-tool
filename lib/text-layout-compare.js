...
function driftBuckets(matched, opts = {}) {
  ...
  const items = matched.map((p, i) => ({
    y: p.src.y, sx: p.src.x, dx: p.tgt.x - p.src.x, dy: p.tgt.y - p.src.y,
    bid: p.src.bid != null ? `b${p.src.bid}` : `s${i}`,
  }));
  // An inline paragraph (runs sharing a block ancestor `bid`) is ONE layout unit, so it
  // gets ONE drift sample: its first run in reading order, whose start sits at the natural
  // left margin and is wrap-stable. This is what a group means — if it reflows internally
  // at a different column width (later runs pushed to other lines), that's the group
  // laying itself out, not drift; if the group genuinely moved, its first run moved too.
  const anchors = new Map();
  for (const it of items) {
    const cur = anchors.get(it.bid);
    if (!cur || it.y < cur.y || (it.y === cur.y && it.sx < cur.sx)) anchors.set(it.bid, it);
  }
  let green = 0;
  let yellow = 0;
  let red = 0;
  for (const it of anchors.values()) {
    // Baseline = median shift of the K nearest runs OUTSIDE this paragraph — the local
    // uniform page shift (including the source↔target vertical gap that grows down the
    // page). The pool is dense (all other-group runs, not just their anchors) so that
    // gap cancels accurately; a paragraph must not define its own baseline.
  ...
  return { green, yellow, red, total: anchors.size };
}
