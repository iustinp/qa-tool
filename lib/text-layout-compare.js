...
function annotateAlignment(src, tgt, opts = {}) {
  ...
  const flush = () => {
    ...
    if (reconcileReflow(seg, reflowCoverageMin)) {
      ...
      // items up so the overlay can treat them like a match — draw a connector
      // and let a click sync/anchor on them.
      const S = seg.src;
      const T = seg.tgt;
      ...
      // Pair by RELATIVE geometry within the group, not raw index. The two sides
      // are the same content re-split, so items line up ROW-WISE by position. Raw
      // index pairing breaks when the sides split into different run COUNTS — e.g.
      // a staff list where the source renders each entry as "Name" + ", Role" (two
      // runs) but the target renders "Name, Role" as one: index k then races through
      // the target twice as fast and every later run collapses onto the last row.
      // Coordinates are made relative to each side's first element to cancel any
      // page-wide offset between the two group origins before matching.
      const nearestIn = (pool, rx, ry) => {
        let best = pool[0].it;
        let bd = Infinity;
        for (const c of pool) {
          const d = Math.abs(c.ry - ry) * 1e6 + Math.abs(c.rx - rx);
          if (d < bd) { bd = d; best = c.it; }
        }
        return best;
      };
      const rel = (arr, x0, y0) => arr.map((it) => ({ it, rx: it.x - x0, ry: it.y - y0 }));
      const Sr = rel(S, S[0].x, S[0].y);
      const Tr = rel(T, T[0].x, T[0].y);
      Sr.forEach(({ it, rx, ry }) => {
        it.status = 'reflow';
        it.counterpart = nearestIn(Tr, rx, ry);
        it.groupFirst = groupFirst;
      });
      Tr.forEach(({ it, rx, ry }) => {
        it.status = 'reflow';
        it.counterpart = nearestIn(Sr, rx, ry);
        it.groupFirst = groupFirst;
      });
    } else {
...
function driftBuckets(matched, opts = {}) {
  ...
  const items = matched.map((p, i) => ({
    y: p.src.y, sx: p.src.x, h: p.src.h || 16, dx: p.tgt.x - p.src.x, dy: p.tgt.y - p.src.y,
    bid: p.src.bid != null ? `b${p.src.bid}` : `s${i}`,
  }));
  // Drift of one run vs the MEDIAN shift of its K nearest neighbours OUTSIDE its paragraph —
  // the local uniform page shift (incl. the source↔target vertical gap that grows down-page).
  // The pool is dense (all other-paragraph runs) so that gap cancels; a paragraph must not
  // define its own baseline.
  const driftOf = (it) => {
    const near = items.filter((o) => o.bid !== it.bid).sort((a, b) => Math.abs(a.y - it.y) - Math.abs(b.y - it.y)).slice(0, K);
    if (!near.length) return 0;
    const mdx = med(near.map((o) => o.dx));
    const mdy = med(near.map((o) => o.dy));
    return Math.max(Math.abs(it.dx - mdx), Math.abs(it.dy - mdy));
  };
  // Group the runs by paragraph (`bid`) and treat two kinds differently — the wrap problem
  // only exists across LINES, so we only group there:
  //   UNFUSED (all runs on ~one line — e.g. a row of footer links separated horizontally):
  //     each run is a distinct element, so grade every run on its own. No hidden grouping.
  //   FUSED (runs span MULTIPLE lines — one flowing, wrapped paragraph): grade ONCE at the
  //     first run; the later runs sit where the paragraph wrapped at a different column width
  //     (reflow, not drift), so they are NOT scored (and are drawn gray in the review).
  // We count per SCORED run, so the green/yellow/red here equal the coloured connectors on
  // the page — what you see is what's counted.
  const byBid = new Map();
  for (const it of items) { const a = byBid.get(it.bid); if (a) a.push(it); else byBid.set(it.bid, [it]); }
  let green = 0;
  let yellow = 0;
  let red = 0;
  const bump = (d) => { if (d <= good) green += 1; else if (d <= okish) yellow += 1; else red += 1; };
  for (const runs of byBid.values()) {
    const ys = runs.map((r) => r.y);
    const h = med(runs.map((r) => r.h)) || 16;
    const multiLine = Math.max(...ys) - Math.min(...ys) > h * 0.5;
    if (multiLine) {
      const anchor = runs.reduce((a, it) => (!a || it.y < a.y || (it.y === a.y && it.sx < a.sx)) ? it : a, null);
      bump(driftOf(anchor));
    } else {
      for (const it of runs) bump(driftOf(it));
    }
  }
