...
function compareTextLayout(source, target, opts = {}) {
  ...
  // ── SCORES (all 0-100, higher = worse, 0 = identical) ────────────────────────
  // Global health decomposes into a content-completeness score and a drift score.
  const clamp = (v) => Math.max(0, Math.min(100, Math.round(v)));
  // Drift: weighted ordinal mean of the R/Y/G buckets (green 0, yellow 1, red 2),
  // scaled to 100. A single red always nudges it up (never lost to rounding).
  ...
  // Content: same weighted-ordinal idea over SOURCE lines — missing 2, extra 1,
  // matched 0 — so a completeness loss always outweighs an equal amount of extra.
  ...
  const contentScore = clamp((2 * missPct + extraPct) / 2);
  const globalScore = clamp(0.7 * contentScore + 0.3 * driftScore);
  ...
  return {
    ...
    // Health scores + drift buckets (the new headline; higher = worse).
    globalScore,
    driftScore,
    contentScore,
