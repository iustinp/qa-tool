...
const { exploreUrl } = require('./recursive-explorer');
    ...
    if (useCanonical) {
      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          // Recursive explorer per side (single page, stacked in-place mutation-undo
          // for backtracking, timer-accelerated stable-text signatures for dedup);
          ...
          // Floor for a "full enough" crawl load, per side, from the base capture's
          // clickable scale — the heavy source site sometimes under-renders on the
          // crawl load (few triggers -> starved coverage); exploreUrl retries when
          // it comes up short of this (~30% of detected clickables).
          const minTrig = (clm) => Math.round(((clm && clm.clickables && clm.clickables.length) || 0) * 0.3);
          const [srcCrawl, tgtCrawl] = await Promise.all([
            exploreUrl(source, { ...crawlOpts, minTriggers: minTrig(srcClm) }),
            exploreUrl(target, { ...crawlOpts, minTriggers: minTrig(tgtClm) }),
          ]);
