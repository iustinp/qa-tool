...
async function processPair(
  ...
  try {
    ...
    // pixelDetect: the second-opinion pixel path (#62 D). Uses cheap VIEWPORT
    // before/after frames scrolled to each trigger. ON by default: a real IDFC run
    // measured ~+40% time (about half the old fullPage overhead) with no volatile/
    // OCR noise, so the cost is acceptable for the occlusion/visual-only reveals it
    // catches. Disable with PPD_PIXEL_DETECT=0; PPD_PIXEL_SCALE overrides the
    // detection downscale (default 0.25; OCR stays full-res).
    const pixelDetect = process.env.PPD_PIXEL_DETECT !== '0' && process.env.PPD_PIXEL_DETECT !== 'false';
