...
async function processPair(
  ...
  try {
    ...
    if (layoutAudit && srcPrimary.textGeometry && tgtPrimary.textGeometry) {
      const layoutT0 = Date.now();
      const layout = compareTextLayout(srcPrimary.textGeometry, tgtPrimary.textGeometry);
      report.layoutAudit = {
      ...
      try {
        const ovT0 = Date.now();
        const alignment = alignForOverlay(srcPrimary.textGeometry, tgtPrimary.textGeometry);
