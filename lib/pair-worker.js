...
- **Only split where a section genuinely changes:** a distinct background band, a new heading that introduces clearly new content, or the footer.
- **Never bleed into the NEXT section.** The bottom edge must stop exactly where the current section ends; do **not** include the next section's heading, cards, or background. "Bigger" means *don't sub-split one section into slivers* — it does **NOT** mean extend past the section's real bottom. Over-including the next section deletes it from later iterations, so at the bottom edge prefer stopping slightly early over bleeding down.`;
...
async function processPair(
  ...
  for (let iter = 1; iter <= maxIterations; iter++) {
    ...
    // Windowed target matching: instead of the whole (very tall, squished-to-
    // 305xN) target, send only a vertical slice around the source band's
    // expected relative position. A shorter slice is far less skinny, so the
    // model localizes top/bottom accurately (no mid-block cuts) and can't grab a
    // far-off neighbour. The window is generous enough to absorb source↔target
    // layout drift. Disable with PPD_MATCH_WINDOW=0.
    const windowEnabled = process.env.PPD_MATCH_WINDOW !== '0';
    const windowFrac = Math.min(1, Math.max(0.2, parseFloat(process.env.PPD_MATCH_WINDOW_FRAC || '0.5')));
    const srcMidRatio = (sourceBboxOnPage.y + sourceBboxOnPage.height / 2) / (fullPageH || 1);
    let windowTop = 0;
    let windowHeight = th;
    if (windowEnabled) {
      const half = Math.max(sourceBboxOnPage.height * 1.5, (th * windowFrac) / 2);
      const center = srcMidRatio * th;
      windowTop = Math.max(0, Math.round(center - half));
      const windowBottom = Math.min(th, Math.round(center + half));
      windowHeight = windowBottom - windowTop;
      if (windowHeight >= th - 4) {
        windowTop = 0;
        windowHeight = th;
      }
    }
    const usingWindow = windowEnabled && windowHeight < th;

    img = Date.now();
    const targetMatchBuf = usingWindow
      ? await extractCrop(targetFull, { x: 0, y: windowTop, width: tw, height: windowHeight })
      : targetFull;
    const tgtVision = await resizeForVision(targetMatchBuf, visionEdges.matchTargetMaxEdge);
    ...
    await dbg('resize_for_match', {
      ...
      windowTop,
      windowHeight,
      windowPctOfPage: th ? Math.round((windowHeight / th) * 100) : 100,
      targetWxH: `${tgtVision.visionWidth}x${tgtVision.visionHeight}`,
      cropWxH: `${cropVision.visionWidth}x${cropVision.visionHeight}`,
    });

    const sliceNote = usingWindow
      ? `\n\nIMPORTANT: the target image is ONLY a vertical slice of the full target page — the portion from ${((windowTop / th) * 100).toFixed(0)}% to ${(((windowTop + windowHeight) / th) * 100).toFixed(0)}% of the full page height (this is where the source block is expected). Return the bbox normalized to THIS slice image (0=slice top, 1=slice bottom). If the source block is not within this slice, return found:false.`
      : '';
    const matchParts = [
      ...
      {
        type: 'text',
        text: `${buildMatchPrompt(report.targetMatchedRegions, sourceBboxOnPage, fullPageW, fullPageH, targetPageHeight)}\n\nCrop image size: ${cropVision.visionWidth}x${cropVision.visionHeight}. Target slice image: ${tgtVision.visionWidth}x${tgtVision.visionHeight}.${sliceNote}`,
      },
    ];
    ...
    if (match && match.found === true && match.bbox) {
      if (usingWindow) {
        // bbox is normalized on the slice image; map into the slice then offset
        // by the slice's top to get full-page target coordinates.
        const inWindow = normToPixelsFull(match.bbox, tw, windowHeight);
        targetRectFull = {
          x: inWindow.x,
          y: windowTop + inWindow.y,
