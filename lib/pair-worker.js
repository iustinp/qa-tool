...
const { crawlUrlParallel } = require('./state-crawler');
    ...
    if (useCanonical) {
      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          // Parallel crawl: N worker pages per side split the triggers and run
          // concurrently. Sides run SEQUENTIALLY so the peak browser count stays
          // at N (each worker is a full browser), not 2N.
          const crawlOpts = { maxTriggers: 120, settleMs: 280, captureStates: true, maxStates: 40, captureOptions: resolvedCaptureOptions };
          const srcCrawl = await crawlUrlParallel(source, crawlOpts);
          const tgtCrawl = await crawlUrlParallel(target, crawlOpts);
          // crawl.json is a lightweight provenance log — strip the heavy per-state
          // pear (clm) and screenshot buffers (persisted separately as PNGs).
          const slimCrawl = (c) => ({
            ...c,
          ...
          report.layoutAudit.crawl = {
            ...
            srcWorkers: srcCrawl.workers,
            tgtWorkers: tgtCrawl.workers,
            srcWorkerErrors: srcCrawl.workerErrors,
            tgtWorkerErrors: tgtCrawl.workerErrors,
            crawlLastError: srcCrawl.lastError || tgtCrawl.lastError || null,
