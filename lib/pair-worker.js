      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          const [srcCrawl, tgtCrawl] = await Promise.all([
            crawlUrl(source, { maxTriggers: 80, settleMs: 280, captureOptions: resolvedCaptureOptions }),
            crawlUrl(target, { maxTriggers: 80, settleMs: 280, captureOptions: resolvedCaptureOptions }),
          ]);
          fs.writeFileSync(path.join(pairDir, 'crawl.json'), JSON.stringify({ source: srcCrawl, target: tgtCrawl }, null, 1));
          const baseNorms = (clm) => new Set((clm.nodes || []).filter((n) => n.kind === 'text').map((n) => norm(n.text)));
          const revealedNorms = (crawl) => new Set(crawl.revealed.map((r) => norm(r.text)));
          const srcBase = baseNorms(srcClm);
          const tgtBase = baseNorms(tgtClm);
          const srcRev = revealedNorms(srcCrawl);
          const tgtRev = revealedNorms(tgtCrawl);
          const tgtRevealed = new Map();
          for (const r of tgtCrawl.revealed) {
            const k = norm(r.text);
            if (k && !tgtRevealed.has(k)) tgtRevealed.set(k, r.triggerLabel);
          }
          // Base "missing on target" that is actually present behind an interaction.
          const present = [];
          for (const m of layout.missing) {
            const k = norm(m.text);
            if (tgtRevealed.has(k)) present.push({ text: m.text, trigger: tgtRevealed.get(k) });
          }
          // Source content only reachable by interaction that the target never shows.
          const missing = [];
          for (const r of srcCrawl.revealed) {
            const k = norm(r.text);
            if (k && !tgtBase.has(k) && !tgtRevealed.has(k)) missing.push({ text: r.text, trigger: r.triggerLabel });
          }
          gated = { present, missing };

          // Group revealed lines by their trigger (for click-to-reveal in the UI),
          // tagging each line present/missing on the OTHER side.
          const groupTriggers = (crawl, otherBase, otherRev) => {
            const map = new Map();
            for (const r of crawl.revealed) {
              ...
              const k = norm(r.text);
              g.revealed.push({ text: r.text, onOther: otherBase.has(k) || otherRev.has(k) ? 'present' : 'missing' });
            }
            return [...map.values()];
          };
          triggers = {
            source: groupTriggers(srcCrawl, tgtBase, tgtRev),
            target: groupTriggers(tgtCrawl, srcBase, srcRev),
          };
