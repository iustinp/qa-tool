    ...
    if (useCanonical) {
      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          // A snap may only pull a trigger within a plausible drift radius; a
          // "match" farther than this is almost certainly the wrong element (a
          // coincidental label whose real target isn't in the base CLM), and
          // snapping to it is worse than leaving the trigger where the crawl saw
          // it — so in that case we keep the crawl coords.
          const MAX_SNAP = 300;
          const snapTrigger = (t, clm) => {
            ...
            const geo = (a) => ({ x: a.x, y: a.y, w: a.w, h: a.h });
            const nearest = (arr) => {
            ...
            const labMatch = (a) => {
              ...
              return key && l && (l === key || l.startsWith(key) || key.startsWith(l));
            };
            if (key) {
              ...
              return geo(t); // labelled but no plausible match → keep crawl coords
            }
            const c = nearest(clks); // label-less (icons/bullets): nearest base clickable, if close
            return c ? geo(c) : geo(t);
          };
          for (const t of triggers.source) Object.assign(t, snapTrigger(t, srcClm));
          for (const t of triggers.target) Object.assign(t, snapTrigger(t, tgtClm));
