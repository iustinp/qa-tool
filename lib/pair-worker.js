...
async function processPair(
  ...
  try {
    ...
    // pixelDetect: the second-opinion pixel path (#62 D). Now uses cheap VIEWPORT
    // before/after frames scrolled to each trigger (the fullPage-per-click cost P4
    // flagged is gone). Still OFF by default until a real run confirms the new cost
    // is affordable — flip on with PPD_PIXEL_DETECT=1 to measure. PPD_PIXEL_SCALE
    // overrides the detection downscale (default 0.25; OCR stays full-res).
