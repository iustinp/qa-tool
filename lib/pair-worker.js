...
async function processPair(
  ...
  try {
    await dbg('capture_start', { source, target });
    const [srcCapture, tgtCapture] = await Promise.all([
      captureFullPageBuffer(source, {
        ...resolvedCaptureOptions,
        captureRole: 'source',
        captureUrl: source,
        logCaptureStep: captureLog,
      }),
      captureFullPageBuffer(target, {
        ...resolvedCaptureOptions,
        captureRole: 'target',
        captureUrl: target,
        logCaptureStep: captureLog,
    ...
    srcBuf = srcCapture.buffer;
    tgtBuf = tgtCapture.buffer;
    sourceMetadata = srcCapture.metadata || null;
    targetMetadata = tgtCapture.metadata || null;
