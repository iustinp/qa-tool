    ...
    if (useCanonical) {
      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          // Persist each captured interaction STATE (pear + full-page shot) and
          // attach it to the trigger group that produced it — matched by the same
          // label@crawl-coord key groupTriggers used, BEFORE snapping mutates the
          // group geometry. The review tool renders the state pear and swaps it in
          // when the trigger's purple box is clicked.
          const statesDir = path.join(pairDir, 'states');
          const attachStates = (crawl, groups, side) => {
            if (!crawl.states || !crawl.states.length) return;
            ensureDir(statesDir);
            const byKey = new Map(groups.map((g) => [`${g.label}@${g.x},${g.y}`, g]));
            crawl.states.forEach((st, i) => {
              const g = byKey.get(`${st.triggerLabel}@${st.triggerX},${st.triggerY}`);
              if (!g) return;
              let shot = null;
              if (st.shot) {
              ...
              // Drop the state pear's inlined fonts — the review head already
              // loads the base page fonts and state pears reuse them (keeps the
              // review HTML from ballooning across many states).
              const clm = st.clm ? { width: st.clm.width, height: st.clm.height, nodes: st.clm.nodes } : null;
              g.state = { shot, clm, revealed: st.revealed || [] };
            });
          };
          attachStates(srcCrawl, triggers.source, 'src');
          attachStates(tgtCrawl, triggers.target, 'tgt');
