...
async function processPair(
  ...
  try {
    ...
    const crawlOpts = layoutCrawl
      ? // pixelDetect stays OFF by default: even with downscaled detection, a full
        // run is ~3x slower — the bottleneck is the full-page SCREENSHOT per
        // text-flat click on a ~10k-px page (+ OCR), not the diff. Turning it on
        // needs viewport/clip-scoped capture (P4 cont.) — see #62. Capability is
        // built + fixture-validated behind opts.pixelDetect.
        { perNodeBudget: 150, maxDepth: 3, maxStates: 150, settleMs: 280, timeBudgetMs: 300000, captureStates: true, captureOptions: resolvedCaptureOptions }
      : null;
