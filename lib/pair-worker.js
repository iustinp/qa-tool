...
async function processPair(
  ...
  for (let iter = 1; iter <= maxIterations; iter++) {
    ...
    // Prompt caching (opt-in): the full target image is identical across every
    // match call in this pair, so put [static base + target image] first and mark
    // a cache breakpoint after the target — subsequent match calls reuse the
    // cached target-image tokens instead of re-uploading them. The varying crop +
    // per-iteration context follow the breakpoint. Default (off) keeps the
    // original crop-first arrangement byte-for-byte.
    ...
    const matchParts = promptCache
      ? [
          { type: 'text', text: MATCH_PROMPT_BASE_CACHED },
          { type: 'image_buffer', buffer: tgtVision.buffer, cache: true },
          { type: 'image_buffer', buffer: cropVision.buffer },
          {
            type: 'text',
            text: `${buildMatchContext(report.targetMatchedRegions, sourceBboxOnPage, fullPageW, fullPageH, targetPageHeight)}\n\n${dims}`,
          },
        ]
      : [
          { type: 'image_buffer', buffer: cropVision.buffer },
    ...
    await dbg('match_api_done', {
      ...
      promptCache,
      cacheReadTokens: matchResult.usage?.cache_read_input_tokens ?? null,
      cacheWriteTokens: matchResult.usage?.cache_creation_input_tokens ?? null,
      inputTokens: matchResult.usage?.input_tokens ?? null,
    });
