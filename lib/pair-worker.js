    ...
    if (useCanonical) {
      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          // Single-page crawl per side (in-place mutation-undo between clicks —
          // no reloads, no parallel tabs); the two sides run concurrently.
          const crawlOpts = { maxTriggers: 120, settleMs: 280, captureStates: true, maxStates: 40, captureOptions: resolvedCaptureOptions };
          const [srcCrawl, tgtCrawl] = await Promise.all([crawlUrl(source, crawlOpts), crawlUrl(target, crawlOpts)]);
