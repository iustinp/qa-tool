...
async function processPair(
  ...
  const {
    ...
    textOnly = false,
    profiles = ['desktop'],
    ignore = [], //        recipe ignore — removed from BOTH sides (shared chrome)
    ignoreSource = [], //  recipe ignore — removed from the SOURCE capture only
    ignoreTarget = [], //  recipe ignore — removed from the TARGET capture only
    cacheStore = null,
    layoutAudit = true,
    layoutOcr = false,
  ...
  // Each matched element (and its subtree) is removed from that side's capture.
  const selectorsForSide = (role) =>
    [...ignore, ...(role === 'target' ? ignoreTarget : ignoreSource)]
    ...
    const captureSide = async (url, role) => {
      ...
      const mkOpts = () => ({
        ...
        captureOptions: crawlOpts
          ...
          : { ...resolvedCaptureOptions, ignoreSelectors },
      });
