    ...
    if (useCanonical) {
      ...
      if (layoutCrawl) {
        ...
        try {
          ...
          const [srcCrawl, tgtCrawl] = await Promise.all([exploreUrl(source, crawlOpts), exploreUrl(target, crawlOpts)]);
          ...
          const graph = { source: buildGraph(srcCrawl, 'src'), target: buildGraph(tgtCrawl, 'tgt') };
