...
const { exploreStates } = require('./recursive-explorer');
const { hashInputs } = require('./cache-store');
const { screenPair, isScreeningEnabled } = require('./pair-screening');
const { auditVisibleText, buildPresenceMatcher, createSkippedTextAudit } = require('./text-audit');
...
async function runPairTextAudit(report, sourceMetadata, targetMetadata, screeningVerdict, dbg) {
  ...
  // HIDDEN SLIDER CONTENT reclassification (#63): pull off-screen slider text OUT
  // of plain content-missing into a dedicated hidden-content bucket, using the
  // hiddenNodes captured by canonical-layout (symmetric both sides). A source
  // hidden-slider line flagged 'missing' (not in target's VISIBLE text) is:
  //  - dropped if it IS present in target's own hidden slider content
  //    (present-on-both, just hidden → nothing to flag, per the missing/extra logic);
  //  - else a genuine hidden-MISSING (source shows it in a slider, target lacks it).
  // EXTRA (on target) = target hidden-slider text absent from source (visible+hidden).
  try {
    const srcHidden = sourceMetadata?.canonicalLayout?.hiddenNodes || [];
    const tgtHidden = targetMetadata?.canonicalLayout?.hiddenNodes || [];
    if (srcHidden.length || tgtHidden.length) {
      // Fuzzy presence matchers (exact/substring/partial-token — same as the main
      // audit) so punctuation/case drift ("cards" vs "cards.", "FIRST" vs "First")
      // doesn't read as absent. Presence is judged against the OTHER side's full
      // content (visible + hidden).
      // Off-screen slides only. The captured set now also includes each slider's
      // currently-VISIBLE slide (for a deterministic, cross-side-symmetric set in the
      // review), but a visible slide is normal on-screen content — its presence/
      // absence is the plain visible audit's job, so it must NOT reclassify content
      // out of the plain buckets. Only truly-hidden slides do that.
      const srcHiddenOff = srcHidden.filter((n) => !n.visibleNow);
      const tgtHiddenOff = tgtHidden.filter((n) => !n.visibleNow);
      const presentOnTarget = buildPresenceMatcher([...targetAuditLines, ...tgtHidden.map((n) => n.text)]);
      const presentOnSource = buildPresenceMatcher([...sourceAuditLines, ...srcHidden.map((n) => n.text)]);
      const isSourceHidden = buildPresenceMatcher(srcHiddenOff.map((n) => n.text));
      // 1) Remove ANY source-hidden line from the plain content-missing — its
      //    presence is now judged (below) against target's hidden content too, not
      //    just target's visible text, so it must not linger in the visible-only bucket.
      report.textAudit.missingLines = report.textAudit.missingLines.filter((m) => !isSourceHidden(m.sourceLine));
      report.textAudit.missingLineCount = report.textAudit.missingLines.length;
      report.textAudit.status = report.textAudit.missingLineCount === 0 ? 'ok' : 'discrepancies';
      // 2) Hidden-MISSING: a source hidden-slider line absent from target (visible+hidden).
      const hiddenMissing = [];
      const seenM = new Set();
      for (const n of srcHiddenOff) {
        const k = normalizeTextLine(n.text || '');
        if (!k || seenM.has(k)) continue;
        seenM.add(k);
        if (presentOnTarget(n.text)) continue; // present on target (visible or hidden) → nothing to flag
        hiddenMissing.push({ text: n.text, norm: k, x: n.x, y: n.y, w: n.w, h: n.h, itemIdx: n.itemIdx, clickable: n.clickable });
      }
      // 3) Hidden-EXTRA: a target hidden-slider line absent from source (visible+hidden).
      const hiddenExtra = [];
      const seenE = new Set();
      for (const n of tgtHiddenOff) {
        const k = normalizeTextLine(n.text || '');
        if (!k || seenE.has(k)) continue;
        seenE.add(k);
        if (presentOnSource(n.text)) continue;
        hiddenExtra.push({ text: n.text, norm: k, x: n.x, y: n.y, w: n.w, h: n.h, itemIdx: n.itemIdx, clickable: n.clickable });
      }
      report.hiddenContent = { missing: hiddenMissing, extra: hiddenExtra };
      await dbg('hidden_content_reclassified', { hiddenMissing: hiddenMissing.length, hiddenExtra: hiddenExtra.length, plainMissingNow: report.textAudit.missingLineCount });
    }
  } catch (e) {
    await dbg('hidden_content_reclassify_error', { error: e.message });
  }
...
async function processPair(
  ...
  const {
    ...
    textOnly = false,
    profiles = ['desktop'],
    ignore = [], //        recipe ignore — marked/hatched on BOTH sides (shared chrome)
    ignoreSource = [], //  recipe ignore — SOURCE only
    ignoreTarget = [], //  recipe ignore — TARGET only
    clickSource = [], //   recipe click-if-present — SOURCE only (e.g. cookie accept)
    clickTarget = [], //   recipe click-if-present — TARGET only
    cacheStore = null,
    layoutAudit = true,
    layoutOcr = false,
  ...
  const toSelectors = (list) =>
    (list || [])
      .map((r) => (typeof r === 'string' ? r : r && r.selector))
      .filter((s) => typeof s === 'string' && s.trim());
  // Recipe ignore selectors for a side: shared `ignore` (both) + that side's list.
  const selectorsForSide = (role) =>
    toSelectors([...ignore, ...(role === 'target' ? ignoreTarget : ignoreSource)]);
  // Recipe click-if-present selectors for a side (e.g. cookie Accept/Deny).
  const clickForSide = (role) => toSelectors(role === 'target' ? clickTarget : clickSource);
  ...
  const report = {
    ...
    reorderedMatches: [],
    extraOnTarget: [],
    hiddenContent: { missing: [], extra: [] }, // #63: off-screen slider content, reclassified out of plain content-missing
    screening: null,
    textAudit: null,
  ...
  try {
    ...
    // ONE load per side: capture the base pear+screenshot AND run the interaction
    // crawl in the SAME page load. captureFullPageBuffer extracts the base CLM +
    // screenshot BEFORE onPageReady, so the comparison pear is clean and pre-crawl;
    // the crawl then explores (mutation-undo between clicks, no reload). This
    // avoids a second rapid load of the same URL (anti-bot) and the cross-load
    // coordinate drift that used to need trigger-snapping. No timer acceleration:
    // it's proven unnecessary for coverage post additive/delta fixes and would
    // pollute the base pear (carousels landing on random slides).
    // Text-flat second opinion (#62). OCCLUSION detection (DOM hit-test) is the
    // DEFAULT — it catches covered-text reveals ~120x cheaper than screenshot+OCR
    // (measured 5ms vs ~600ms/click). The PIXEL path is now the explicit FALLBACK,
    // OFF by default (PPD_PIXEL_DETECT=1) for the cases occlusion can't see:
    // pointer-events:none opaque overlays and non-text visual reveals.
    const occlusionDetect = process.env.PPD_OCCLUSION_DETECT !== '0' && process.env.PPD_OCCLUSION_DETECT !== 'false';
    const pixelDetect = process.env.PPD_PIXEL_DETECT === '1' || process.env.PPD_PIXEL_DETECT === 'true';
    const pixelScale = process.env.PPD_PIXEL_SCALE ? parseFloat(process.env.PPD_PIXEL_SCALE) : 0.25;
    // Crawl bounds. Raised (#62) now that nested triggers are tagged (nav included)
    // + navigation is sandboxed: a heavy site (source) has many more triggers, so
    // the old 300s/150-state budget starved it of depth-2 content recursion. All
    // env-tunable: PPD_CRAWL_TIME_MS, PPD_CRAWL_MAX_STATES, PPD_CRAWL_MAX_DEPTH,
    // PPD_CRAWL_NODE_BUDGET.
    const intEnv = (k, dflt) => (process.env[k] ? parseInt(process.env[k], 10) : dflt);
    const crawlOpts = layoutCrawl
      ? {
          perNodeBudget: intEnv('PPD_CRAWL_NODE_BUDGET', 200),
          maxDepth: intEnv('PPD_CRAWL_MAX_DEPTH', 3),
          maxStates: intEnv('PPD_CRAWL_MAX_STATES', 400),
          // settle after each click before harvesting (280->200 default: most
          // reveals are near-instant DOM mutations; the slowest are rare) and the
          // stable-text sample count/interval (6/120 -> 5/100). Both tunable; both
          // trade a little robustness for speed (#62 timing: settle + sample were
          // ~270s combined). PPD_CRAWL_SETTLE_MS / PPD_CRAWL_SAMPLES / _SAMPLE_MS.
          settleMs: intEnv('PPD_CRAWL_SETTLE_MS', 200),
          sample: { samples: intEnv('PPD_CRAWL_SAMPLES', 5), intervalMs: intEnv('PPD_CRAWL_SAMPLE_MS', 100) },
          timeBudgetMs: intEnv('PPD_CRAWL_TIME_MS', 900000),
          captureStates: true,
          occlusionDetect,
          pixelDetect,
          pixelScale,
      ...
      : null;
    const captureSide = async (url, role) => {
      const ignoreSelectors = selectorsForSide(role);
      const clickSelectors = clickForSide(role);
      const mkOpts = () => ({
        role,
        profiles,
        ...
        captureOptions: crawlOpts
          ? {
              ...resolvedCaptureOptions,
              ignoreSelectors,
              clickSelectors,
              // PPD_CLICK_DEBUG: route the crawl's per-tag/per-click trace into
              // run-debug.log (event "clickdbg", role-tagged) so it's inspectable
              // without capturing stderr. grep run-debug.log for clickdbg.
              onPageReady: (page) => exploreStates(page, { ...crawlOpts, clickLog: (msg) => dbg('clickdbg', { role, msg }) }),
            }
          : { ...resolvedCaptureOptions, ignoreSelectors, clickSelectors },
      });
      let cap = await capturePage(url, mkOpts());
      // #60: if the crawl under-rendered (few triggers vs the clickables the base
      // capture detected), the load was likely incomplete (heavy anti-bot) — retry
      // the whole load once and keep the richer result.
      if (crawlOpts) {
        const prim = primaryProfile(cap);
        const clickables = (((prim.canonicalLayout && prim.canonicalLayout.clickables) || []).length);
        const trig = (prim.onPageReadyResult && prim.onPageReadyResult.triggersTotal) || 0;
        if (clickables > 0 && trig < clickables * 0.3) {
          await dbg('crawl_underrendered_retry', { role, trig, clickables });
          const retry = await capturePage(url, mkOpts());
          const rtrig = (primaryProfile(retry).onPageReadyResult && primaryProfile(retry).onPageReadyResult.triggersTotal) || 0;
          if (rtrig > trig) cap = retry;
        }
      }
      return cap;
    };
    const [sourceCapture, targetCapture] = await Promise.all([captureSide(source, 'source'), captureSide(target, 'target')]);
    ...
    // Per-resolution screenshots: each profile keeps its own <side>-<name>-full.png.
    // Only when there's more than one profile — single-res runs keep just the
    // unsuffixed files (byte-identical to before).
    const profileNames = Object.keys(sourceCapture.profiles);
    const primaryName = primaryProfileName(sourceCapture);
    const multiRes = profileNames.length > 1;
    if (multiRes) {
      for (const [name, p] of Object.entries(sourceCapture.profiles)) {
        if (p.buffer) fs.writeFileSync(path.join(shotsDir, `source-${name}-full.png`), p.buffer);
      }
      for (const [name, p] of Object.entries(targetCapture.profiles)) {
        if (p.buffer) fs.writeFileSync(path.join(shotsDir, `target-${name}-full.png`), p.buffer);
      }
    }
    ...
    // Shape a compareCanonical/compareTextLayout result into the report's audit
    // summary (shared by the primary block and the per-resolution loop below).
    const toLayoutSummary = (layout, geoSourceLabel) => ({
      ...
      contentScore: layout.contentScore,
      driftScore: layout.driftScore,
      driftGreen: layout.driftGreen,
      driftYellow: layout.driftYellow,
      driftRed: layout.driftRed,
      geoSource: geoSourceLabel,
    ...
    // Canonical layout audit for a single non-primary resolution: writes its own
    // clm/audit/review artifacts (suffixed by profile name) and returns the summary.
    const auditProfileCanonical = async (name, srcClmP, tgtClmP) => {
      try {
        await computeImageHashes(path.join(shotsDir, `source-${name}-full.png`), srcClmP.nodes);
        await computeImageHashes(path.join(shotsDir, `target-${name}-full.png`), tgtClmP.nodes);
      } catch (e) {
        await dbg('profile_image_hash_error', { profile: name, error: e.message });
      }
      const { audit: layout, alignment } = compareCanonical(srcClmP, tgtClmP);
      fs.writeFileSync(
        path.join(pairDir, `layout-audit-${name}.json`),
        JSON.stringify(
          { capturedAt: new Date().toISOString(), sourceUrl: source, targetUrl: target, geoSource: 'canonical', profile: name, ...layout },
          null,
          2
        )
      );
      fs.writeFileSync(path.join(pairDir, `${name}-source-clm.json`), JSON.stringify(srcClmP));
      fs.writeFileSync(path.join(pairDir, `${name}-target-clm.json`), JSON.stringify(tgtClmP));
      try {
        const html = buildCanonicalReviewHtml({
          ...
          sourceClm: srcClmP,
          targetClm: tgtClmP,
          sourceDims: { w: srcClmP.width, h: srcClmP.height },
          targetDims: { w: tgtClmP.width, h: tgtClmP.height },
          sourceImg: `screenshots/source-${name}-full.png`,
          targetImg: `screenshots/target-${name}-full.png`,
          alignment,
          audit: layout,
          gated: null,
          triggers: null,
        ...
        fs.writeFileSync(path.join(pairDir, `layout-review-${name}.html`), html);
      } catch (e) {
        await dbg('profile_review_error', { profile: name, error: e.message });
      }
      return toLayoutSummary(layout, 'canonical');
    };
    ...
    if (useCanonical) {
      ...
      // Optional recursive interaction crawl: explore the page as a STATE GRAPH
      // (click -> new/known/noop -> recurse), revealing content gated behind
      // clicks (tabs/accordions/modals) at any depth, and resolve it against the
      // base audit. Discovery-tree parent linkage + per-edge trigger geometry
      // drive the hierarchical breadcrumb review.
      let gated = null;
      let triggers = null;
      let graph = null;
      if (layoutCrawl) {
        ...
        try {
          ...
          // The crawl already ran during the base capture (onPageReady, same load);
          // read each side's result off the primary profile.
          const emptyCrawl = { states: [], edges: [], revealed: [], baseSignature: null, statesFound: 0, triggersClicked: 0, triggersTotal: 0, navigations: 0, reloads: 0 };
          const srcCrawl = srcPrimary.onPageReadyResult || emptyCrawl;
          const tgtCrawl = tgtPrimary.onPageReadyResult || emptyCrawl;
          // crawl.json is a lightweight provenance log — strip the heavy per-state
          // pear (clm) and screenshot buffers (persisted separately as PNGs), but
          // keep the graph shape (signatures, parent linkage, edges) the review needs.
          const slimCrawl = (c) => ({
            baseSignature: c.baseSignature,
            statesFound: c.statesFound,
            triggersClicked: c.triggersClicked,
          ...
          fs.writeFileSync(path.join(pairDir, 'crawl.json'), JSON.stringify({ source: slimCrawl(srcCrawl), target: slimCrawl(tgtCrawl) }, null, 1));
          ...
          // Build the interaction STATE GRAPH (discovery tree) for the hierarchical
          // breadcrumb review: persist each captured state's full-page screenshot,
          // key states by signature, and hang each state's child triggers (its
          // outgoing 'new' edges) with their geometry in THAT state's own document
          // space — so deep-state boxes align with the state's own backdrop and need
          // no snapping (only base-level boxes reuse the base-CLM snap below).
          const statesDir = path.join(pairDir, 'states');
          const buildGraph = (crawl, side, presentFn) => {
            const states = {};
            // All captured state signatures, known upfront so a 'known' edge can be
            // included only when its target state actually exists (states{} is built
            // incrementally below, so it can't be consulted mid-build).
            const capturedSigs = new Set((crawl.states || []).map((s) => s.signature));
            // Clickable child boxes for a given parent state. 'new' AND 'known' edges both
            // lead to a REAL captured state, so both are navigable. Include 'known' so a
            // trigger whose revealed content deduped to an already-visited state — e.g. the
            // Lifestyle tab, whose new<->known outcome flips run to run — still gets a box
            // instead of vanishing. But a 'known' edge only earns a box at a FRESH spot:
            // skip degenerate (0,0) geometry, and skip any position already occupied by
            // another box (several nav items / FAQ accordions collide onto ONE state at ONE
            // recorded point and would otherwise stack dozens of boxes). 'noop'/'nav' edges
            // (no distinct state) are excluded.
            const mkChild = (e) => ({ sig: e.toSig, label: e.triggerLabel, x: e.triggerX, y: e.triggerY, w: e.triggerW, h: e.triggerH, hidden: e.hidden, ghost: e.ghost });
            // Cards in a carousel/grid ROW share a top edge. A slider clicked right after a
            // tab-swap is measured MID-TRANSITION, so its active (usually first) slide records
            // a top ~53px off while its settled siblings agree — the "first card box painted
            // lower" bug, in states especially (state children aren't CLM-snapped). Snap each
            // same-size horizontal cluster's y to the row MEDIAN (2+ settled siblings outvote
            // the stray one). Framework- and timing-blind; rows are >150px tall & clusters use
            // an 80px band, so distinct rows (>300px apart) never merge.
            const normalizeRows = (kids) => {
              const med = (a) => { const s = [...a].sort((x, y) => x - y); const m = s.length >> 1; return s.length % 2 ? s[m] : Math.round((s[m - 1] + s[m]) / 2); };
              const groups = {};
              for (const c of kids) {
                if (c.w < 150 || c.h < 150) continue; // card-ish only
              ...
              for (const arr of Object.values(groups)) {
                ...
                while (i < arr.length) {
                  const cluster = [arr[i]];
                  let j = i + 1;
                  while (j < arr.length && arr[j].y - cluster[cluster.length - 1].y <= 80) { cluster.push(arr[j]); j += 1; }
                  if (cluster.length >= 2 && new Set(cluster.map((c) => Math.round(c.x / 8))).size >= 2) {
                    const my = med(cluster.map((c) => c.y));
                    for (const c of cluster) c.y = my;
                  }
                  i = j;
                }
              }
            ...
            // Correct each child box's Y to the state SCREENSHOT's scroll regime using the
            // rects snapshotted at capture (childRects, doc coords at captureScrollY). A
            // sticky header makes absolute Y scroll-dependent, so an edge's click-time Y can
            // be ~53px off the shot; match by the SCROLL-INVARIANT x/w/h and adopt the
            // snapshot Y. This is the root fix (vs normalizeRows, which only patched
            // horizontal rows and couldn't touch a uniformly-shifted row or a vertical list).
            const correctY = (kids, rects) => {
              if (!Array.isArray(rects) || !rects.length) return kids;
              for (const k of kids) {
                let bestY = null;
                let bestD = Infinity;
                for (const r of rects) {
                  if (Math.abs(r[2] - k.w) > 6 || Math.abs(r[3] - k.h) > 10 || Math.abs(r[0] - k.x) > 8) continue;
                  const d = Math.abs(r[1] - k.y);
                  if (d < bestD) { bestD = d; bestY = r[1]; }
                }
                if (bestY != null && bestD <= 120) k.y = bestY;
              }
              return kids;
            };
            const childrenFrom = (fromSig) => {
              // Drop SELF-EDGES (toSig === fromSig): the click revealed nothing new so the
              // crawl looped back to this same state. As a 'known' child box it would
              // re-enter the current state on click — an endless "deeper" descent that never
              // changes (the "Load more" self-loop). A real navigable child must go elsewhere.
              const es = (crawl.edges || []).filter((e) => e.fromSig === fromSig && e.toSig !== fromSig);
              const news = es.filter((e) => e.kind === 'new').map(mkChild);
              const posKey = (c) => `${Math.round(c.x / 10)},${Math.round(c.y / 10)}`;
              const occupied = new Set(news.map(posKey));
              const knowns = [];
              for (const e of es) {
                if (e.kind !== 'known' || !capturedSigs.has(e.toSig)) continue;
                const c = mkChild(e);
                if (c.x < 2 || c.y < 2) continue; // degenerate capture (0 on EITHER axis = bad geometry)
                const k = posKey(c);
                if (occupied.has(k)) continue; // don't stack on an existing box
                occupied.add(k);
                knowns.push(c);
              }
              return normalizeRows([...news, ...knowns]);
            };
            (crawl.states || []).forEach((st, i) => {
              if (crawl.states.length) ensureDir(statesDir);
              let shot = null;
              if (st.shot) {
                const rel = path.posix.join('states', `${side}-${i}.png`);
                fs.writeFileSync(path.join(pairDir, rel), st.shot);
                shot = rel;
              }
              const kids = correctY(childrenFrom(st.signature), st.childRects);
              // Newly-revealed text lines for the state view: captured in the shot's scroll
              // regime, tagged present/missing on the OTHER side so the review colours each
              // green (this-side-only), red (other-side-only), or blue (both).
              const revealText = (Array.isArray(st.revealText) ? st.revealText : []).map((l) => ({
                x: l[0], y: l[1], w: l[2], h: l[3], onOther: presentFn(l[4]) ? 'present' : 'missing',
              }));
              states[st.signature] = {
                id: `${side}-${i}`,
                parent: st.parentSignature,
                label: st.triggerLabel,
                depth: st.depth,
                shot,
                revealText,
                // #62 Phase B — the backdrop is a VIEWPORT shot (viewport-at-
                // clickable), so the state-view canvas is the viewport frame, and
                // scrollY lets the review map doc-space child boxes into it.
                // Fall back to full-doc CLM dims for older/full-page captures.
                w: st.viewportWidth || (st.clm && st.clm.width) || null,
                h: st.viewportHeight || (st.clm && st.clm.height) || null,
                scrollY: st.captureScrollY || 0,
                children: kids,
              };
            });
            const base = crawl.baseSignature;
            const baseChildren = childrenFrom(base);
            return { base, states, baseChildren };
          };
          graph = { source: buildGraph(srcCrawl, 'src', onTarget), target: buildGraph(tgtCrawl, 'tgt', onSource) };
          // Link each base-level trigger GROUP to the child state it opens (matched
          // by the same label@raw-coord key groupTriggers used — BEFORE snapping
          // mutates the group geometry), so clicking a base purple box descends into
          // that state. Deeper descents come from graph.states[sig].children.
          const linkBaseTriggers = (graphSide, groups) => {
            const byKey = new Map(groups.map((g) => [`${g.label}@${g.x},${g.y}`, g]));
            for (const c of graphSide.baseChildren) {
              const g = byKey.get(`${c.label}@${c.x},${c.y}`);
              if (g) g.sig = c.sig;
            }
          };
          linkBaseTriggers(graph.source, triggers.source);
          linkBaseTriggers(graph.target, triggers.target);
          ...
          const labelSnap = (t, clm) => {
            ...
            // A UNIQUE, distinctive label match is never a coincidence, so trust
            // it even across a big layout shift (a carousel / lazy section can
            // sit far from where the crawl saw it — e.g. a blog card ~850px off).
            // The MAX_SNAP cap only guards SHORT or AMBIGUOUS labels, where the
            // nearest of several same-label candidates is the safe pick.
            // Distinctive = a unique base match that's a real phrase: long, or
            // multi-word. A unique 2-word label ("other links") is safe to trust
            // across a big shift (a footer accordion ~950px down); short single
            // words keep the MAX_SNAP cap. "Multi-word" means ALPHABETIC words, not
            // just whitespace: a generic carousel ordinal like "2 / 4" has spaces but
            // is shared by EVERY 4-item carousel on the page, so it must NOT snap
            // across distance (it wrongly dragged a hero slide's box+ghost onto the
            // Key Benefits "2 / 4" card 6500px away). Require real letters.
            const DISTINCTIVE = 12;
            const words = key.match(/[a-z]{2,}/gi) || [];
            const distinctive = words.length >= 2 || (words.length >= 1 && key.length >= DISTINCTIVE);
            const pick = (arr) => {
              if (!arr.length) return null;
              const best = arr.slice().sort((a, b) => dist(a) - dist(b))[0];
              if (arr.length === 1 && distinctive) return best;
            ...
            const labMatch = (a) => {
              ...
              // Prefix relation handles trigger label = title+excerpt vs base
              // title (or vice versa) — but only when the SHORTER side is itself
              // a substantial phrase, so a stray 1-char node ("m") can't match a
              // long label and break uniqueness.
              const short = l.length < key.length ? l : key;
              return short.length >= 5 && (l.startsWith(key) || key.startsWith(l));
            };
            const c = pick(clks.filter(labMatch));
            if (c) return { x: c.x, y: c.y, w: c.w, h: c.h };
            const n = pick(texts.filter(labMatch));
            if (n) {
              const mx = n.x + n.w / 2;
              const my = n.y + n.h / 2;
          ...
          const snapTriggers = (list, clm) => {
            // Geometric fallback for a trigger that can't LABEL-match (a carousel card
            // often has an empty/generic label): snap it to a base CLM clickable of the
            // SAME size sitting almost exactly on its raw crawl coord. Such a card's crawl
            // coord is already correct, so this pins it in place instead of letting it
            // inherit a neighbour's region drift — which was pushing the first slider
            // card(s) ~53px below where they render in the screenshot. The tight centre
            // tolerance + size match mean it can only grab the card's own box (row cards
            // are >300px apart), never an unrelated one.
            const geomSnap = (t) => {
              const tcx = t.x + t.w / 2;
              const tcy = t.y + t.h / 2;
              let best = null;
              let bestD = Infinity;
              for (const c of clm.clickables || []) {
                if (Math.abs(c.w - t.w) > t.w * 0.15 || Math.abs(c.h - t.h) > t.h * 0.15) continue;
                const d = Math.hypot(c.x + c.w / 2 - tcx, c.y + c.h / 2 - tcy);
                if (d < bestD) { bestD = d; best = c; }
              }
              return best && bestD <= 60 ? { x: best.x, y: best.y, w: best.w, h: best.h } : null;
            };
            const anchors = [];
            const pending = [];
            for (const t of list) {
              const g = labelSnap(t, clm) || geomSnap(t);
              const ocx = t.x + t.w / 2;
              const ocy = t.y + t.h / 2;
              if (g) {
          ...
          // #62 — the base-view SOLID clickable boxes are drawn from the state
          // graph's `baseChildren`, which carry RAW crawl coords just like the
          // trigger groups. With the crawl now at a TALL viewport, a vh-sized
          // section grows and pushes those coords down (a "Read More" trigger
          // drifted 1512px = the extra viewport height), so the solid boxes landed
          // far below where they belong on the base pear while the dashed base-CLM
          // markers stayed put. Snap baseChildren onto base-CLM geometry too, so
          // the solid boxes coincide with the (correctly placed) dashed markers.
          // State-view `children` are deliberately NOT snapped: they're drawn over
          // the crawl-layout viewport screenshot and are self-consistent there.
          snapTriggers(graph.source.baseChildren, srcClm);
          snapTriggers(graph.target.baseChildren, tgtClm);
          ...
          report.layoutAudit.crawl = {
            ...
            srcReloads: srcCrawl.reloads,
            tgtReloads: tgtCrawl.reloads,
      ...
      try {
        const html = buildCanonicalReviewHtml({
          ...
          triggers,
          graph,
          hiddenContent: report.hiddenContent, // #63: off-screen slider content + missing/extra
        });
      ...
      // --- Per-resolution layout audits. The primary is already in report.layoutAudit;
      // compute the other resolutions and expose all, keyed by profile, so the reports
      // can tab by resolution. (Crawl + OCR stay primary-only for now.) Single-res
      // runs skip the suffixed files entirely (unchanged on disk).
      report.layoutAuditByProfile = { [primaryName]: report.layoutAudit };
      if (multiRes) {
        // Give the primary a resolution-suffixed set too, for uniform tabs.
        try {
          for (const [from, to] of [
            ['layout-audit.json', `layout-audit-${primaryName}.json`],
            ['source-clm.json', `${primaryName}-source-clm.json`],
            ['target-clm.json', `${primaryName}-target-clm.json`],
            ['layout-review.html', `layout-review-${primaryName}.html`],
          ]) {
            const src = path.join(pairDir, from);
            if (fs.existsSync(src)) fs.copyFileSync(src, path.join(pairDir, to));
          }
        } catch (e) {
          await dbg('primary_alias_error', { error: e.message });
        }
        for (const name of profileNames) {
          if (name === primaryName) continue;
          const sc = sourceCapture.profiles[name] && sourceCapture.profiles[name].canonicalLayout;
          const tc = targetCapture.profiles[name] && targetCapture.profiles[name].canonicalLayout;
          if (!sc || !tc) continue;
          try {
            report.layoutAuditByProfile[name] = await auditProfileCanonical(name, sc, tc);
          } catch (e) {
            await dbg('profile_layout_audit_error', { profile: name, error: e.message });
          }
        }
      }
    ...
    if (report.interactionManifest.regions.length > 0) {
      ...
      const [srcReplay, tgtReplay] = await Promise.all([
        replayInteractionManifestForText(source, report.interactionManifest, {
          ...resolvedCaptureOptions,
          captureRole: 'source',
          ignoreSelectors: selectorsForSide('source'),
          logCaptureStep: captureLog,
        }),
        replayInteractionManifestForText(target, report.interactionManifest, {
          ...resolvedCaptureOptions,
          captureRole: 'target',
          ignoreSelectors: selectorsForSide('target'),
          logCaptureStep: captureLog,
        }),
      ]);
