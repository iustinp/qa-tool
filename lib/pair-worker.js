...
/**
 * Remainder-match acquisition: enumerate candidate target regions for a source
 * block (the target has already-matched regions blanked), then, if more than one
 * candidate, make a disambiguation call to pick the one that is genuinely the
 * same section. Returns a synthesized single `match` ({found, confidence, bbox,
 * rationale}) so the existing acceptance/record flow is reused unchanged.
 */
async function acquireRemainderMatch(o) {
  const candParts = [
    { type: 'image_buffer', buffer: o.cropVision.buffer },
    { type: 'image_buffer', buffer: o.tgtVision.buffer },
    {
      type: 'text',
      text: `${buildCandidatesPrompt(o.sourceBboxOnPage, o.fullPageW, o.fullPageH, o.targetPageHeight)}\n\n${o.dims}`,
    },
  ];
  const t0 = Date.now();
  const res = await cachedVisionJson(o.cacheStore, candParts, 2048);
  let candidates = Array.isArray(res.parsed?.candidates) ? res.parsed.candidates : [];
  candidates = candidates
    .map((c) => ({ ...c, bbox: normalizeBbox(c.bbox) }))
    .filter((c) => isUsableBbox(c.bbox));
  await o.dbg('remainder_candidates', {
    iter: o.iter,
    ms: Date.now() - t0,
    count: candidates.length,
    confidences: candidates.map((c) => c.confidence ?? null),
  });

  if (candidates.length === 0) {
    return { match: { found: false, confidence: 0, rationale: 'no candidates' }, matchMs: Date.now() - t0 };
  }
  if (candidates.length === 1) {
    const c = candidates[0];
    return {
      match: { found: true, confidence: c.confidence ?? 0.9, bbox: c.bbox, rationale: c.rationale || null },
      matchMs: Date.now() - t0,
    };
  }

  // >1 candidate: dedicated disambiguation call.
  await o.rateLimiter.acquire();
  const disParts = [
    { type: 'image_buffer', buffer: o.cropVision.buffer },
    { type: 'image_buffer', buffer: o.tgtVision.buffer },
    {
      type: 'text',
      text: `${buildDisambiguatePrompt(candidates, o.sourceBboxOnPage, o.fullPageW, o.fullPageH, o.targetPageHeight)}\n\n${o.dims}`,
    },
  ];
  const dT0 = Date.now();
  const disRes = await cachedVisionJson(o.cacheStore, disParts, 1024);
  const idx = Number.isInteger(disRes.parsed?.chosenIndex) ? disRes.parsed.chosenIndex : -1;
  await o.dbg('remainder_disambiguate', {
    iter: o.iter,
    ms: Date.now() - dT0,
    candidateCount: candidates.length,
    chosenIndex: idx,
    confidence: disRes.parsed?.confidence ?? null,
  });
  const matchMs = Date.now() - t0;
  if (idx < 0 || idx >= candidates.length) {
    return {
      match: { found: false, confidence: disRes.parsed?.confidence ?? 0, rationale: disRes.parsed?.rationale || 'none chosen' },
      matchMs,
    };
  }
  ...
  return {
    match: {
      found: true,
      confidence: disRes.parsed?.confidence ?? chosen.confidence ?? 0.9,
      bbox: chosen.bbox,
      rationale: disRes.parsed?.rationale || chosen.rationale || null,
    },
    matchMs,
  };
}
...
async function processPair(
  ...
  // Remainder mode: the candidate/disambiguation framing deflates the model's
  // confidence vs the direct "is this THE match?" prompt, so accepted matches
  // (positionally aligned) were failing the 0.85 gate. Use a lower, configurable
  // floor for this mode; the disambiguation call is the real commitment.
  const remainderMatchThresholds = {
    ...matchThresholds,
    minConfidence: parseFloat(process.env.PPD_REMAINDER_MATCH_MIN_CONFIDENCE || '0.6'),
    minConfidenceIfReordered: parseFloat(
      process.env.PPD_REMAINDER_MATCH_MIN_CONFIDENCE_REORDERED || '0.7'
    ),
  };
  ...
  for (let iter = 1; iter <= maxIterations; iter++) {
    ...
    img = Date.now();
    // Remainder-match: blank already-matched regions out of the target IN PLACE
    // (full dimensions preserved, so recorded bboxes stay in full-page space).
    const targetMatchBuffer = remainderMatch
      ? await blankMatchedRegions(targetFull, report.targetMatchedRegions)
      : targetFull;
    const tgtVision = await resizeForVision(targetMatchBuffer, visionEdges.matchTargetMaxEdge);
    ...
    const dims = `Crop image size: ${cropVision.visionWidth}x${cropVision.visionHeight}. Full target page: ${tgtVision.visionWidth}x${tgtVision.visionHeight}.`;

    let matchResult;
    let match;
    let matchMs;
    if (remainderMatch) {
      // Enumerate candidates on the blanked target, disambiguate if >1, and
      // synthesize a single match for the existing acceptance/record flow.
      const rm = await acquireRemainderMatch({
        cropVision,
        tgtVision,
        sourceBboxOnPage,
        fullPageW,
      ...
      match = rm.match;
      matchMs = rm.matchMs;
      matchResult = { usage: null, rawText: null };
    } else {
      const matchParts = [
      ...
      matchResult = await cachedVisionJson(cacheStore, matchParts, 2048);
      matchMs = Date.now() - matchT0;
      match = matchResult.parsed;
    }
    ...
    if (targetRectFull) {
      matchAcceptance = evaluateMatchAcceptance({
        ...
        targetPageHeight,
        // In remainder mode matched regions are blanked out of the target, so the
        // IoU-with-prior gate is redundant — skip it (pass no priors).
        priorMatchedRegions: remainderMatch ? [] : report.targetMatchedRegions,
        thresholds: remainderMatch ? remainderMatchThresholds : matchThresholds,
      });
