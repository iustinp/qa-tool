...
async function harvestVisibleTextInPage(page) {
  ...
  const json = await page.evaluate(() => {
    // A click can start a navigation; mid-navigation document.body is briefly null
    // and createTreeWalker would throw. Guard it so a harvest during a nav yields
    // [] instead of crashing the whole capture (the crawl's nav check then reloads).
...
/**
 * OCCLUSION-AWARE harvest of the CURRENT VIEWPORT: returns the visible text lines
 * that are actually the TOPMOST thing at their own on-screen location, via the
 * browser's own hit-test (document.elementFromPoint). Unlike the display/opacity
 * check, this catches OCCLUSION — text that is display:block but covered by an
 * opaque overlay reads as NOT visible here (elementFromPoint returns the overlay),
 * exactly as a user perceives it. A ~120x-cheaper replacement for the screenshot+
 * OCR pixel path for text reveals (#62): ~5ms vs ~600ms. VIEWPORT-ONLY —
 * elementFromPoint needs client coords, so off-viewport text is skipped (the crawl
 * scrolls each trigger into view before checking). Returns a normalized (norm)
 * de-duplicated array. Caveat: a `pointer-events:none` opaque overlay is seen
 * THROUGH (hit-test ignores it) — the pixel path remains the fallback for that.
 */
async function harvestVisibleViewport(page) {
  const json = await page.evaluate(() => {
    if (!document.body) return JSON.stringify([]);
    ...
    for (let n = w.nextNode(); n; n = w.nextNode()) {
      ...
      if (cx < 0 || cy < 0 || cx > vw || cy > vh) continue; // off-viewport: can't hit-test
      const top = document.elementFromPoint(cx, cy);
      if (!top) continue;
      // Visible = the text's element is on the same ancestor line as the topmost
      // element at the point (no separate overlay between them).
  ...
  // Normalise + de-dup on the Node side (same norm the crawl compares with).
  const nseen = new Set();
  const res = [];
  for (const t of arr) {
    const k = norm(t);
    if (k.length >= 2 && !nseen.has(k)) {
      nseen.add(k);
      res.push({ raw: t, k });
    }
  }
