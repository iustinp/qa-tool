...
async function harvestVisibleTextInPage(page) {
  ...
  const json = await page.evaluate(() => {
    ...
    for (let n = w.nextNode(); n; n = w.nextNode()) {
      ...
      if (!vis(el)) continue;
      // Position of the text itself (Range rect), in document coords — feeds the
      // position-delta change signal. Cheap; ignored by the text-set path. Skip
      // position for fixed/sticky text: its document coord shifts with any scroll,
      // so it would masquerade as "moved" — never a real rearrangement (f flag).
      let x = 0;
      let y = 0;
      let f = false;
      try {
        ...
        x = Math.round(rr.left + window.scrollX);
        y = Math.round(rr.top + window.scrollY);
        for (let p = el; p; p = p.parentElement) {
...
/**
 * Lines that MOVED between two position maps, relative to the page as a whole.
 * We subtract the MEDIAN displacement of all shared lines, so a uniform reflow
 * (an accordion above pushes everything down by the same amount) scores ~0 and
 * is NOT flagged — only a line that moves DIFFERENTLY from its neighbours (a
 * genuine rearrangement: reorder, grid/list toggle) exceeds moveTol. Reuses the
 * local-vs-global drift idea from the review overlay. Needs a few shared lines to
 * establish a reliable median.
 * @param {Object<string,{x,y}>} refPos
 * @param {Object<string,{x,y}>} curPos
 */
function relMoved(refPos, curPos, opts = {}) {
  const moveTol = opts.moveTol ?? 12;
  const shared = Object.keys(refPos || {}).filter((k) => curPos && curPos[k]);
  if (shared.length < 4) return [];
  const dxm = median(shared.map((k) => curPos[k].x - refPos[k].x));
  const dym = median(shared.map((k) => curPos[k].y - refPos[k].y));
  const moved = [];
  for (const k of shared) {
    const rx = curPos[k].x - refPos[k].x - dxm;
    const ry = curPos[k].y - refPos[k].y - dym;
    if (Math.max(Math.abs(rx), Math.abs(ry)) > moveTol) moved.push(k);
  }
  return moved;
}
...
async function sampleStableText(page, opts = {}) {
  ...
  const jitterTol = opts.jitterTol ?? 4;
  const arr = []; // each sample: [{ t, x, y }]
  for (let i = 0; i < samples; i++) {
    arr.push(await harvestVisibleTextInPage(page));
    if (i < samples - 1) await page.waitForTimeout(intervalMs);
  }
  // Stable TEXT set (present-in-all), as before — the primary, reflow-robust signal.
  const { stable, volatile } = stableFromSamples(arr.map((s) => s.map((o) => o.t)));
  // Position map for stable lines: the median (x,y) — but ONLY for lines whose
  // position is itself consistent across samples (spread <= jitterTol). A line
  // that jitters (animating, reflowing) is position-volatile and gets no entry,
  // so it can never fire the moved signal. Unique stable lines only (skip dupes).
  ...
  for (const sample of arr) {
    ...
    for (const o of sample) {
      const k = norm(o.t);
      if (!stableSet.has(k) || seenInSample.has(k) || o.f) continue; // skip fixed/sticky
      seenInSample.add(k);
      if (!perLine.has(k)) perLine.set(k, []);
      perLine.get(k).push(o);
    }
  }
  const posMap = {};
  for (const [k, pts] of perLine) {
    if (pts.length < arr.length) continue; // not positioned in every sample
    const xs = pts.map((p) => p.x);
    const ys = pts.map((p) => p.y);
    const spread = Math.max(Math.max(...xs) - Math.min(...xs), Math.max(...ys) - Math.min(...ys));
    if (spread > jitterTol) continue; // position-volatile → exclude from moved detection
    posMap[k] = { x: median(xs), y: median(ys) };
  }
  return { stable, volatile, posMap };
}
 ...
 */
function changedFrom(refStable, curStable, opts = {}, refPos = null, curPos = null) {
  ...
  // Complementary position-delta: existing lines that rearranged (reorder /
  // layout toggle) — a change the text-set is blind to. Reflow-masked by relMoved.
  const moved = refPos && curPos ? relMoved(refPos, curPos, opts) : [];
  return { changed: !sameState(refStable, curStable, opts), added, removed, moved };
}
