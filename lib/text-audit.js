...
/**
 * Reusable presence test with the SAME exact/substring/partial-token logic as
 * auditVisibleText — returns (line) => boolean ("is this line present anywhere in
 * targetLines"). Factored out so other comparisons (e.g. hidden slider content,
 * #63) tolerate punctuation/wording drift identically to the main content audit
 * (a trailing '.' or "FIRST" vs "First" must not read as absent).
 */
function buildPresenceMatcher(targetLines, opts = {}) {
  const minSubstringLength = opts.minSubstringLength ?? 8;
  const minCoverage =
    opts.minTokenCoverage ?? parseFloat(process.env.PPD_TEXT_MATCH_MIN_COVERAGE || '0.8');
  const minCoverageTokens =
    opts.minTokenCoverageTokens ?? parseInt(process.env.PPD_TEXT_MATCH_MIN_TOKENS || '4', 10);
  const { exactSet, substringBlob, tokenBlob } = buildTargetIndexes(targetLines);
  return (line) => {
    const normalizedLine = normalizeTextLine(line || '');
    if (!normalizedLine) return false;
    if (exactSet.has(normalizedLine)) return true;
    ...
    const tokens = normalizedSubstring ? normalizedSubstring.split(' ') : [];
    if (tokens.length >= minCoverageTokens && longestContiguousTokenCoverage(tokens, tokenBlob) >= minCoverage) {
      return true;
    }
    return false;
  };
