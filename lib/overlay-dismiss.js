...
async function dismissOverlaysOnPage(page, options = {}) {
  ...
  for (const visualOverlay of sorted) {
    ...
    try {
      ...
      await delay(300);
      // The dismiss click can trigger a full-page navigation on some sites (many
      // consent-management platforms reload the page once consent is recorded, to
      // apply tracking scripts) — wait it out so callers don't proceed against a
      // mid-navigation document (document.body is transiently null while the new
      // document parses). Resolves immediately if no navigation was triggered.
